...
#+latex_header_extra: \newglossaryentry{context}{name={context},description={A sequence of “variable ∶ type” declarations; a dictionarry associating variables to types; c.f., record-type and object-oriented class}}
#+latex_header_extra: \newglossaryentry{signature}{name={signature},description={A sequence of pairs of name-type declarations; an alias for ‘context’ and ‘telescope’}}
#+latex_header_extra: \newglossaryentry{module-systems}{name={Module systems},description={Module systems parameterise programs, proofs, and tactics over structures}}
...
typechecked code, is presented later in §ref:sec:what_is_DTL.
...
#+latex: \noindent
We shall explain this idea more concretely via two examples, in the following
two sections. The Agda fragments presented will be explained in the accompanying
text ---an introduction to Agda is given in §ref:sec:what_is_DTL. Afterword, we
conclude by briefly mentioning theoretical concerns when working with DTLs and,
more importantly for topic on modularisation, issues of a more practical nature
...
*** Example 2: Correct-by-Construction Programming
 ...
 cite:sop,sop_cohen,ewd_discipline: Each proof of a program property acts as
...
*** The Curry-Howard Correspondence ---“Propositions as Types”
     ...
     language a also a proof assistant: A proposition is proved by writing a
...
*** The trials and tribulations of working with dependent types
 ...
 order to be determined cite:why_dependent_types_matter. A glaring drawback is
 ...
 cite:undecidability_of_typing. E.g., later we shall define the type ~Vec A n~ of
...
:Comparision:
 ...
 cite:dtl_implementation_practical,dtl_implementation_simple,dtl_implementation_tutorial,dtl_implementation_idris,dtl_implementation_lectures_and_code.
 In particular, after more than 30 years after Martin-{{{lof}}}'s work on the
 type theory cite:lof_constructive_math,lof_itt, it is still unclear how such
 ...
 2. Do we allow the “Type in Type” axiom
     cite:russell_type_hierarchy,agda_type_in_type_contradiction,system_F_with_type_in_type,extended_cic?

 3. What about “Axiom K” expressing /almost/ the recursion scheme of identity types
    cite:uip_streicher,dependent_matching_is_just_K,matching_without_K,eliminating_dependent_matching,elimination_with_motive,uip_problem,uip_strength}?
 ...
 5. Should unification be proof-relevant; i.e., to consider the /ways/ in which
    terms can be made equal cite:proof_relevant_unification?

 6. How do subtypes, which classically require proof irrelevance,
    tie into the paradigm?

 7. How does proof-term erasure work cite:dtl_practical_erasure,dtl_index_erasure,erasure_type_systems,proof_irrelevant_cic}?
 ...
 cite:ornaments_relationally,tt_in_colour,ornaments. So much for the DRY[fn:2]
...
open HasMonoid {{...}} using (pop-Id-tc)
 ...
 ---otherwise one must use named instances cite:named_instances. Notice that the
...
pop-Id-Σ {{M}} x y = cong (_⨾ y) (rid {x})
 ...
 projections _*directly*_ with possibly meaningful names./ Of course this could be
...
Carrierₜ = proj₁
 ...
 such view at will cite:casl_overview and so programs may be written according
...
same by definition. As such, we are forced to “coe”rce back and forth; leaving
the distributivity axiom as an exotic property of addition, multiplication, and
coercions. Even worse, without the cleverness of declaring two coercion helpers,
the typing of ~distribute₀~ would have been so large and confusing that the
concept would be rendered near useless.

Let's recall what equality means. One says ~𝓁 ≡ 𝓇~ is <<</definitionally equal/>>>
...
earlier since the shared carrier is declared upfront.
...
# #
The ~open ⋯ public ⋯ renaming ⋯~ pattern shown below will be presented in a future
section as a library method.
...
with additive symbols, other times with multiplicative symbols.
...
woah : Monoid
woah = record { semigroup = record { Carrier = {!!}
                                   ; _⨾_     = {!!}
                                   ; assoc   = {!!}
                                   }
              ; Id      = {!!}
...
* TODO COMMENT Current Approaches
   :PROPERTIES:
...
** COMMENT Expectations of Module Systems
...
** COMMENT Ad hoc Grouping Mechanisms
<<<sec:module_ad_hoc>>>
...
** COMMENT Theory Presentations: A Structuring Mechanism
 <<<sec:module_theory>>>
...
:End:
 ...
 also be relativised.
 :End:
** COMMENT “JSON is Foundational”: From Prototypes to Classes
