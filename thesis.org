...
§ref:sec:lost, as a library method.
...
carrier M = Semigroup.Carrier
 ...
 details”. We shall return to this in a future section.
  ...
  base levels in the hierarchy ---c.f., §ref:sec:examples:redundancy. Another
...
*** COMMENT Excerption
  ...
  # #+latex: \noindent
   ...
   nested theory ---as was the case in §ref:sec:examples:extensions for
...
# +BEGIN_CENTER org
#+caption: Muliple Forms of the Template-Instantiation Duality
...
§ref:sec:DTL:sanitising_raw_data, ref:sec:DTL:correct_by_construction. The Agda
fragments presented will be explained in the accompanying text ---an
introduction to Agda is given in §ref:sec:what_is_DTL. Afterword, we conclude by
...
|-----------------+-----------------------------------------------------------------|
| Dependent types | Uniform treatment of values and types, §ref:sec:DTL:uniformity  |
|                 | and increased expressivity of ‘comments’, §ref:sec:curry_howard |
|-----------------+-----------------------------------------------------------------|
 ...
 presented at the end of this section, §ref:lost. Moreover, we demonstrate a
 ...
 ---such as the =Semigroupᵢ= forms from the previous section, §ref:lost.
...
* COMMENT [!] §4? curapr ← Solution Requirements
   :PROPERTIES:
...
# #+latex: \noindent
 ...
 §ref:sec:examples:IsX. More concretely, suppose we have a binary predicate named
...
translate₀ {B} AS f surj inj = record { Carrier = B ; _⨾_ = _⨾_ ; assoc = assoc }
 ...
 we shall propose in a later section, §ref:sec:lost. More generally, notice that
...
is covered in §ref:sec:module_existing.
