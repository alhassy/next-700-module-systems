...
| /An introduction to Agda and dependent types can be found in §ref:sec:what_is_DTL/ |
...
thought of as its own language cite:modular_modules ---for example, it comes
...
cite:roman20:profun:lenses:prisms:optics to generate such accessors by
...
implementations of Coq cite:coq_cat_experiences and Agda cite:perna, the culprit
...
module system (§ ref:sec:four). Along the way, we intend to provide a set of
...
for a concept ---such as typeclass derivation cite:deriving_via--- users use
...
+ *§ref:sec:examples_from_the_wild Examples from the wild*
...
+ *§ref:sec:metaprogramming_module_meta_primitives Metaprogramming Module Meta-primitives*
   ...
   from §ref:sec:examples_from_the_wild can be tackled.
...
+ *§ref:sec:module_meta_primitives_as_library_methods Module Meta-primitives as Library Methods*
...
+ *§ref:sec:conclusion Conclusion: The lingua franca dream as reality*
...
motto is not flawless; e.g., consider cite:DBLP:conf/afp/McBride04 the Haskell
...
research, we shall first consider the possible candidates. After settling on
Agda, we provide a quick tutorial on the language and on dependent types.
Finally, we conclude with demonstrating our observation of “all packaging
mechanisms are essentially the same” formally through Agda examples by
...
*** Agda --“Haskell on steroids”
  Agda cite:agda_overview,agda_thesis is one of the more popular proof
  ...
  in mind cite:agda_web,agda_plf.
...
*** Coq ---“The standard proof assistant”
  ...
  Coq cite:coq_implementation,coq_cat_experiences is unquestionably one of, if
  not, the most popular proof assistant around. It has been used to produce
  mechanised proofs of the infamous Four Colour Theorem cite:coq_four_colour,
  the Feit-Thompson Theorem cite:coq_feit, and an optimising compiler for the C
...
*** Idris ---“Agda with tactics”
  ...
  rather than theorem proving” cite:idris_faq,idris_tdd. However, like Coq, Idris
...
*** {{{fstar}}} ---“The immature adult”
  The {{{fstar}}} cite:fstar_website language supports dependent types, refinement types,
...
*** Beluga
  ...
  direct support for first-class contexts cite:beluga. A term ~t(x)~ may have free
...
*** Notable Mentions
 ...
 + In contrast to Beluga, Isabelle is a full-featured language and logical
   ...
   ‘locales’ cite:locales,isabelle_locales; unfortunately it is not a
 ...
 + Twelf cite:twelf_site is a logic programming language
   ...
   cite:lf_meta_mechanisation,lf_has_isabelle,lf_fast_proof_checking
 ...
 + Matita cite:matita_main,matita_site is a Coq-like system that is much lighter cite:matita_is_coq_light;
...
identifying types with terms, the type of small types is a larger type; e.g., ~ℕ
∶ Set₀~ and ~Setᵢ ∶ Setᵢ₊₁~ ---the indices ~i~ are called /levels/ and the small type
~Set₀~ is abbreviated as ~Set~. In some regard, Agda adds /harmonious/ support for
...
be liberal with whitespace: ~e∶τ~ is a valid identifier whereas ~e ∶ τ~ declares ~e~
to be of type ~τ~. Agda's Emacs interface allows entering Unicode symbols in
traditional LaTeX-style; e.g., ~\_7, \McN, \to, ~\::~ are replaced by ~₇, 𝒩, →, ∷~.
Moreover, the Emacs interface allows programming by gradual refinement of
incomplete type-correct terms. One uses the “hole” marker ~?~ as a placeholder
...
data Vec {ℓ : Level} (A : Set ℓ) : ℕ → Set ℓ where
 ...
 have a type ~Vec A n~. One says ~Vec~ is /parametrised/ by ~A~ (and ℓ), and /indexed/ by
...
*** COMMENT The Curry-Howard Correspondence ---“Propositions as Types”
     ...
     The [[gls:curry-howard][Curry-Howard Correspondence]] makes Agda a programming language and a
     proof assistant: A proposition is proved by writing a program of the
     corresponding type.
...
*** COMMENT Propositional Equality
...
*** COMMENT Modules ---Namespace Management
...
point   ex₂ = 3
  ...
  Start with ~ex₂ = ?~, then in the hole enter ~C-c C-c RET~ to obtain the /co-pattern/
...
** TODO COMMENT Facets of Structuring Mechanisms: An Agda Rendition
   :PROPERTIES:
 ...
 *directly* rather than use the syntactic sugar of records.
 ...
 It may be thought of as a constructive analogue to the classical set comprehension {{{newline}}} ~{ x ∶ A❙B x}~.
...
** TODO COMMENT Modules in Coq then in Agda
  ...
  parlance. Such module types are also known as ‘functors’ by Coq and Ocaml users;
  which are “generative”: Invocations generate new datatypes. Perhaps an example
  will make this rather strange concept more apparent.
...
** COMMENT Adding Zero then Multiplying by One Results in a Type Error
...
** COMMENT From ~Is𝓧~ to ~𝓧~ ---Packing away components
...
** COMMENT Redundancy, Derived Features, and Feature Exclusion
 ...
 *Extension Design Pattern:* To extend a structure ~𝒳~ by new features ~f₀, …, fₙ~
 ...
 #+begin_src agda :tangle semigroups_to_monoids.agda :prologue module list-is-not-vec where \nopen import Notation\n
record Semigroup : Set₁ where
  field
    Carrier : Set
    _⨾_     : Carrier → Carrier → Carrier
...
** COMMENT Summary of Some Design Patterns in Dependently-Typed Programming
   :PROPERTIES:
...
long after, and far from, its definition. Mixins ‘mix in’ new functionality by
...
// [1] An object instance; i.e., the dictionary literal {name: "Gödel", age: 12}
...
// [2, 4] A module parameterised by another module that is a “submodule” of “Person”.
...
structures’ cite:coq_canonical, which not only generalise the previously
...
** Theory Presentations: A Structuring Mechanism
 ...
 treated formally under the name of ‘theory presentations’. It is the aim of
 ...
 stratified’ forms of grouping, besides intended use. The two approaches have
 ...
 down to whether we want a ‘module parameter’ or a ‘record field’ ---why not
...
magma.
 ...
 transport along views cite:little_theories: A view =V ∶ 𝒮 → 𝒯= gives rise to a
 term homomorphism ~𝒱~ from ~P~-terms to ~Q~-terms that is type-preserving in that
 whenever =Θ, 𝒮 ⊢ e ∶ τ= then =Θ, 𝒯 ⊢ 𝒱 e : 𝒱 τ=. Thus, views preserve judgements
 ...
 | ~𝒱 x ≈ x~                              | Provided ~x~ is an 𝒮-variable symbol       |
 | ~𝒱(f e₁ … eₙ) ≈ (β f) (𝒱 e₁) … (𝒱 eₙ)~ | Provided ~f~ is an ~n~-ary 𝒮-function symbol |
 | ~𝒱(𝒬 x • P) ≈ (𝒬 x ∣ U x • 𝒱 P)~       | Provided ~𝒬~ is a variable-binder ~∀, ∃, λ~  |
#+latex:  \end{tcolorbox}
...
:End:
 ...
 sufficient conditions for a translation to be an ‘interpretation’ which
 ...
 propositions-as-types view means we mechanically provide certain “proofs of
 propositions”. Incidentally, moving forward, we shall consider an essentially
...
*** Conclusion
   ...
   abbreviation: ~𝓍 instanceof 𝓎 ≈ Object.getPrototypeOf(𝓍) == 𝓎.prototype~.
...
* TODO COMMENT Solution Requirements
   :PROPERTIES:
...
term ::=  x                    -- variable
 ...
 This is not so with the term {{{newline}}} ~Maybe (A × Vec B n)~ ---~A~ and ~B~ are types while ~n~ is a number.
...
* TODO COMMENT The Second Choice: PackageFormer
...
2. The string icon ~PackageFormer (•̀ᴗ•́)و~ is displayed in the mode line ---near the bottom of Emacs.
...
which is identical to ~CommutativeMonoid₀~ along with a forgetful operation 😄
...
** Defining a Concept Only Once
   ...
   From a library-designer's perspective, our definition of ~CommutativeMonoid~ has the commutativity property ‘hard coded’ into it.
   ...
   Instead, the system lets us ‘build upon’ the ~extended-by~ combinator: We make an associative list of
   names and properties, then string-replace the meta-names /op, op′, rel/ with the provided user names.
...
#+END_SRC
    ...
    + Regarding the idea that /“each piece of mathematical knowledge should be formalized only once”,/
...
# 𝒱-go it = :alter-elements (λ _ → (parse-elements (list (format "%s" 'it))))
# M = Magma go noice
...
Remember, /this particular user implementation/ realises ~X₁ union X₂ :renaming₁ f′ :renaming₂ g′~
as the pushout of the inclusions ~f′ X₁ ∩ g′ X₂ ⟶ Xᵢ~ where the source is the set-wise intersection of names.
Moreover, when either ~renamingᵢ~ is omitted, it defaults to the identity function.
...
:tests:
(flip "_+_" (car (parse-elements '("_+_ : A → B → C"))))  ⇒  _+_ : B → A → C
(flip "_+_" (car (parse-elements '("_⊕_ : A → B → C"))))  ⇒  _⊕_ : A → B → C
...
** COMMENT User Manual II: Extending the System
 ...
 :Hide:
 #+BEGIN_SRC emacs-lisp :tangle "variationals.tmp" :noweb-ref std-𝒱-lib :noweb yes
...
*** COMMENT Substructure
 ...
 {-  lisp
 ;;   f : A → A → A
 ;; ⇒ f-closure : ∀ {x₁ x₂} → ℬ x₁ → ℬ x₂ → ℬ (f x₁ x₂)
 ;;
 ;; (predicate-closure (car (parse-elements '("f : A → A → A"))))
 ;;
 (defun predicate-closure (e)
 (let* ((indices (number-sequence 1 (1- (length (s-split "→" (element-type e))))))
   ...
   (concat (s-replace "_" "" (element-name e))
           ...
           (unless (s-blank-p args)
             (format "∀ {%s} → %s → "
               args
               (s-join " → " (--map (format "ℬ x%s" it) indices))))
     (format "ℬ (%s.%s 𝒜 %s)"
       $𝑝𝑎𝑟𝑒𝑛𝑡
       (element-name e)
       args))))

 (𝒱 substructure carrier
 = "C.f. 𝒱-data ^_^"
  :alter-elements (λ es →
     (thread-last es
       (--filter (s-contains? carrier (target (element-type it))))
       (mapcar #'predicate-closure)
       (cons (format "ℬ : %s.%s 𝒜 → Set" $𝑝𝑎𝑟𝑒𝑛𝑡 carrier))
       (cons (format "𝒜 : %s" $𝑝𝑎𝑟𝑒𝑛𝑡))
       parse-elements)))
 -}

 {-700
 SubMonoid = MonoidR substructure "Carrier" ⟴ record
 -}
*** Lisp Interface
    :PROPERTIES:
...
*** Empty Variationals
    ...
    :END:
 {{{goal(A summary of the key features of 𝒱ariationals)}}}
...
-}
    ...
    and /elaborated/ Lisp of this definition using ~C-h o RET 𝒱-identity~.

    *Remember*: The prefix ~𝒱-~ only occurs at the definition site, the call site omits it.
...
-}
  ...
  hover over ~MonoidPⁱᵈ~ to see its elaboration is precisely that of ~MonoidP~.
...
-}
 ...
 Momentarily, “⟴” is forwards composition: We ‘thread’ the ~Pf~ through the variationals ~vᵢ~ in order.
...
-}
 #+END_SRC
    ...
    *Huh?*
    ...
    In Agda syntax this corresponds to: ~λ es → map (λ it → (map-qualifier (λ _ → "field") it)) es~.
    \\
...
record Monoid-Record-cons : Set₁ where
 ...
 #+BEGIN_SRC emacs-lisp emacs-lisp :tangle "variationals.tmp" :noweb-ref std-𝒱-lib :noweb yes
...
⇒
 ...
 #+BEGIN_SRC emacs-lisp  :tangle "variationals.tmp" :noweb yes :noweb-ref std-𝒱-lib
...
-}
 ...
 #+BEGIN_SRC lisp :tangle "variationals.tmp" :noweb yes :noweb-ref std-𝒱-lib
(𝒱 unbundling n
...
-}
 ...
 #+BEGIN_SRC results-agda :tangle no
record M-Set′-attempt : Set₁ where
   field Scalar′        : Set
   field Vector′        : Set
   field _·′_       : Scalar → Vector → Vector
   field 𝟙′     : Scalar
...
*** Variationals with Arguments ---~map, rename, [co]decorated, subscriptedᵢ, renaming~
    :PROPERTIES:
    :CUSTOM_ID: Variationals-with-Arguments-----map--rename---co-decorated--subscriptedᵢ--renaming-
    :END:
...
(𝒱 typeclass (height 1) (level 'dec)
  = "Expose the first element and decrese the package's level ---c.f. Haskell typeclass.
    ...
    "
    record ⟴ :waist height :level level)
     #+END_SRC
     :End:
     #+BEGIN_src agda
{-700
...
(𝒱 map elements (support-mixfix-names nil) (adjoin-retract nil) (adjoin-coretract nil)
 ...
 2. 700-comments have single-line ~𝒱-name args = rhs~, whereas lisp-blocks
    have multi-line ~(𝒱 name args = rhs)~ ---the dash after the 𝒱 is gone and outer-most
...
"
 ...
 We may also prefer writing ~subscriptedᵢ~ rather than ~decorated "ᵢ"~.
...
record NearMonoid : Set₁ where
 ...
 0. Example ~M₁₂~ demonstrates that composition, ⟴, is sequential from left to right.
 ...
 1. Notice that the ~NearMonoid~ example demonstrates multiplicity of PackageFormer elements is irrelevant.
    That is, elements are algebraically a free monoid with the axiom ~xs ⊕ ys ⊕ xs ≈ xs ⊕ ys~.
...
*** Shallow Renaming with Agda's ~open ⋯ public ⋯ renaming ⋯~
    :PROPERTIES:
...
*** Automatically deriving homomorphism definitions ♥‿♥
    :PROPERTIES:
...
*** Implementation Matter
 ...
 ‘tactic sledgehammers’ cite:tactics, it has also become an attractive
...
| (/usr/local/texlive/2016/texmf-dist/tex/latex/glossaries/base/glossaries.sty                   |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| (/usr/local/texlive/2016/texmf-dist/tex/latex/mfirstuc/mfirstuc.sty)                           |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/textcase/textcase.sty)                           |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/xfor/xfor.sty)                                   |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/datatool/datatool-base.sty                       |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/substr/substr.sty)                               |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/datatool/datatool-fp.sty                         |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp.sty                                        |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| `Fixed                                                                                         | Point                                                                       | Package',                                                      | Version                                         | 0.8                    | April             | 2                                       | 1995        | (C)                |          Michael | Mehlich  |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/defpattern.sty)                               |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-basic.sty)                                 |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-addons.sty)                                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-snap.sty)                                  |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-exp.sty)                                   |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-trigo.sty)                                 |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-pas.sty)                                   |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-random.sty)                                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-eqn.sty)                                   |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-upn.sty)                                   |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/fp/fp-eval.sty))))                               |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/glossaries/base/glossaries-compat                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ible-307.sty)                                                                                  |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/glossaries/styles/glossary-hypern                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| av.sty)                                                                                        |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/glossaries/styles/glossary-list.s                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ty)                                                                                            |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/glossaries/styles/glossary-long.s                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ty)                                                                                            |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/glossaries/styles/glossary-super.                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| sty                                                                                            | (/usr/local/texlive/2016/texmf-dist/tex/latex/supertabular/supertabular.sty |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ))                                                                                             |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/glossaries/styles/glossary-tree.s                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ty))                                                                                           | (./unicode.sty                                                              |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/newunicodechar/newunicodechar.sty                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| )                                                                                              | (/usr/local/texlive/2016/texmf-dist/tex/latex/psnfss/pifont.sty             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/psnfss/upzd.fd)                                  |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/psnfss/upsy.fd))                                 |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| (/usr/local/texlive/2016/texmf-dist/tex/latex/amsmath/amscd.sty)                               |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/tools/enumerate.sty)                             |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/bbm-macros/bbm.sty)                              |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/etex-pkg/etex.sty)                               |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/units/nicefrac.sty)                              |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/jknapltx/mathrsfs.sty)                           |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              443 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              444 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              445 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              455 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              459 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              460 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              482 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              488 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              500 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              548 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              549 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              551 |          |           |
| Package                                                                                        | newunicodechar                                                              | Warning:                                                       | Redefining                                      | Unicode                | character         | on                                      | input       | line               |              552 |          |           |
| )                                                                                              | (/usr/local/texlive/2016/texmf-dist/tex/latex/amsfonts/eufrak.sty           |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| (/usr/local/texlive/2016/texmf-dist/tex/latex/tcolorbox/tcolorbox.sty                          |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| -0-65.sty)                                                                                     |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/pgf/compatibility/pgfcomp-version                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| -1-18.sty))                                                                                    | (/usr/local/texlive/2016/texmf-dist/tex/latex/tools/verbatim.sty)           |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| (/usr/local/texlive/2016/texmf-dist/tex/latex/biblatex/blx-natbib.def)                         |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/biblatex/bbx/alphabetic.bbx                      |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/biblatex/bbx/standard.bbx))                      |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/biblatex/cbx/alphabetic.cbx)                     |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/biblatex/biblatex.cfg))/usr/local/bin/pygmentize |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| (./thesis.bbl)                                                                                 | (/usr/local/texlive/2016/texmf-dist/tex/latex/amsfonts/umsa.fd)             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| (./_minted-thesis/569958E3CFCC4425864289143433A3C73E61B1C84D3334B6C3ED857B56598                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| F16.pygtex)                                                                                    | [1{/usr/local/texlive/2016/texmf-var/fonts/map/pdftex/updmap/pdftex         |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| .map}]pdfTeX                                                                                   | warning                                                                     | (ext4):                                                        | destination                                     | with                   | the               | same                                    | identifier  | (name{page.1}      |                  |          |           |
...
| Chapter                                                                                        | 1                                                                           |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| [2]                                                                                            |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/E04AE6B244FCDB61D8BA14A17F64F69600BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/8F9CCF5820A2725241265CCDAB8AB0A53E61B1C84D3334B6C3ED857B56598                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| F16.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/88D4E59C78F144B9C793F60C97E8B02D3E61B1C84D3334B6C3ED857B56598                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| F16.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/06C25FE0C80B8959051A62F8F034710A00BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/72DC3FCEBB5AB53690D4EA55B021095600BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/D2CD4AAF21EC908398737F330808A96D00BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/BBADE449A2D1372B8E49B7B47F8D56A900BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/783BA666E9BE770299AC4AFF2C7F32B100BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/1C2766C0203CA8489F178B9955964C5400BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/A340936F39B11DB0FCBAD812A56F076C00BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/9EDA79DFBA9C174477F10FBD9C41B88A00BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/20A882C5C27D7D7200ED5F431204314F3E61B1C84D3334B6C3ED857B56598                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| F16.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/2C8C9759D4BF4AC392BEA195273EECD83E61B1C84D3334B6C3ED857B56598                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| F16.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/20A882C5C27D7D7200ED5F431204314F3E61B1C84D3334B6C3ED857B56598                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| F16.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/2C8C9759D4BF4AC392BEA195273EECD83E61B1C84D3334B6C3ED857B56598                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| F16.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/6E5BCB2BF1F6C957FA2A97F3CE1183573E61B1C84D3334B6C3ED857B56598                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| F16.pygtex)                                                                                    |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./_minted-thesis/B74DBA7707EAC25E35ADDEA795B45FF900BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    | [3]                                                                         | [4]                                                            |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| LaTeX                                                                                          | Warning:                                                                    | Reference                                                      | `sec:lost'                                      | on                     | page              | 5                                       | undefined   | on                 |            input | line     |       424 |
| [5]                                                                                            | [6]                                                                         | (/usr/local/texlive/2016/texmf-dist/tex/latex/base/ts1cmtt.fd) |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| LaTeX                                                                                          | Warning:                                                                    | Reference                                                      | `sec:three'                                     | on                     | page              | 7                                       | undefined   | on                 |            input | line     |       541 |
| LaTeX                                                                                          | Warning:                                                                    | Reference                                                      | `sec:four'                                      | on                     | page              | 7                                       | undefined   | on                 |            input | line     |       544 |
| [7pdfTeX                                                                                       | warning                                                                     | (ext4):                                                        | destination                                     | with                   | the               | same                                    | identifier  | (name{table.1.1})  |                  |          |           |
| has                                                                                            | been                                                                        | already                                                        | used,                                           | duplicate              | ignored           |                                         |             |                    |                  |          |           |
| \AtBegShi@Output                                                                               | ...ipout                                                                    | \box                                                           | \AtBeginShipoutBox                              |                        |                   |                                         |             |                    |                  |          |           |
| \fi                                                                                            | \fi                                                                         |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| l.555                                                                                          |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ]                                                                                              |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| LaTeX                                                                                          | Warning:                                                                    | Reference                                                      | `sec:metaprogramming_module_meta_primitives'    | on                     | page              | 8                                       |             |                    |                  |          |           |
| undefined                                                                                      | on                                                                          | input                                                          | line                                            | 595                    |                   |                                         |             |                    |                  |          |           |
| [8]                                                                                            |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| LaTeX                                                                                          | Warning:                                                                    | Reference                                                      | `sec:examples_from_the_wild'                    | on                     | page              | 9                                       | undefined   | on                 |               in |          |           |
| put                                                                                            | line                                                                        | 612                                                            |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| LaTeX                                                                                          | Warning:                                                                    | Reference                                                      | `sec:metaprogramming_module_meta_primitives'    | on                     | page              | 9                                       |             |                    |                  |          |           |
| undefined                                                                                      | on                                                                          | input                                                          | line                                            | 621                    |                   |                                         |             |                    |                  |          |           |
| LaTeX                                                                                          | Warning:                                                                    | Reference                                                      | `sec:examples_from_the_wild'                    | on                     | page              | 9                                       | undefined   | on                 |               in |          |           |
| put                                                                                            | line                                                                        | 627                                                            |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| LaTeX                                                                                          | Warning:                                                                    | Reference                                                      | `sec:module_meta_primitives_as_library_methods' | on                     | pag               |                                         |             |                    |                  |          |           |
| e                                                                                              | 9                                                                           | undefined                                                      | on                                              | input                  | line              | 634                                     |             |                    |                  |          |           |
| LaTeX                                                                                          | Warning:                                                                    | Reference                                                      | `sec:conclusion'                                | on                     | page              | 9                                       | undefined   | on                 |            input | line     |       644 |
| .                                                                                              |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| [9]                                                                                            |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| Chapter                                                                                        | 2                                                                           |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/base/t1cmss.fd)                                  | (./thesis.gls                                                               |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| [10])                                                                                          | [11]                                                                        |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| Overfull                                                                                       | \hbox                                                                       | (7.61392pt                                                     | too                                             | wide)                  | in                | paragraph                               | at          | lines              |         672--672 |          |           |
| \T1/cmr/m/it/12                                                                                | Per-for-mant                                                                | Category-Theory                                                | Li-brary                                        | in                     | Coq\T1/cmr/m/n/12 | .                                       | 20          |                    |                  |          |           |
| 14                                                                                             | arXiv:                                                                      | [][]$\T1/cmtt/m/n/12                                           | 1401                                            | .                      | 7694v2$           | [math.CT][][]\T1/cmr/m/n/12             | .           |                    |                  |          |           |
| [12]                                                                                           | [13]                                                                        | (./thesis.aux)                                                 |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| -dist/fonts/type1/public/amsfonts/cm/cmmi10.pfb></usr/local/texlive/2016/texmf-                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| dist/fonts/type1/public/amsfonts/cm/cmr12.pfb></usr/local/texlive/2016/texmf-di                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| st/fonts/type1/public/amsfonts/cm/cmr8.pfb></usr/local/texlive/2016/texmf-dist/                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| fonts/type1/public/amsfonts/cm/cmsy10.pfb></usr/local/texlive/2016/texmf-dist/f                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| onts/type1/public/cm-super/sfbx1200.pfb></usr/local/texlive/2016/texmf-dist/fon                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ts/type1/public/cm-super/sfbx1728.pfb></usr/local/texlive/2016/texmf-dist/fonts                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| /type1/public/cm-super/sfbx2488.pfb></usr/local/texlive/2016/texmf-dist/fonts/t                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ype1/public/cm-super/sfcc1200.pfb></usr/local/texlive/2016/texmf-dist/fonts/typ                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| e1/public/cm-super/sfit1000.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| /public/cm-super/sfit1200.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/p                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ublic/cm-super/sfrm1095.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/pub                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| lic/cm-super/sfrm1200.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/publi                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| c/cm-super/sfrm1728.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| cm-super/sfrm2488.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| -super/sfsx2488.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-s                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| uper/sfti1200.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-sup                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| er/sftt1000.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-super                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| /sftt1095.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-super/s                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ftt1200.pfb>                                                                                   |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| Output                                                                                         | written                                                                     | on                                                             | thesis.pdf                                      | (15                    | pages,            | 346545                                  | bytes).     |                    |                  |          |           |
...
| Scanning                                                                                       | input                                                                       | file                                                           | thesis.glo....done                              | (3                     | entries           | accepted,                               | 0           | rejected).         |                  |          |           |
| Sorting                                                                                        | entries....done                                                             | (4                                                             | comparisons).                                   |                        |                   |                                         |             |                    |                  |          |           |
| Generating                                                                                     | output                                                                      | file                                                           | thesis.gls....done                              | (12                    | lines             | written,                                | 0           | warnings).         |                  |          |           |
...
| (/usr/local/texlive/2016/texmf-dist/tex/latex/biblatex/biblatex.cfg))/usr/local/bin/pygmentize |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/biblatex/lbx/english.lbx)                        |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./thesis.aux)                                                                                 | (/usr/local/texlive/2016/texmf-dist/tex/latex/base/ts1cmr.fd)               |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/context/base/mkii/supp-pdf.mkii                        |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| [Loading                                                                                       | MPS                                                                         | to                                                             | PDF                                             | converter              | (version          | 2006.09.02).]                           |             |                    |                  |          |           |
| )                                                                                              | (/usr/local/texlive/2016/texmf-dist/tex/latex/oberdiek/epstopdf-base.sty    |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/oberdiek/grfext.sty)                             |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| (/usr/local/texlive/2016/texmf-dist/tex/latex/hyperref/nameref.sty                             |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/generic/oberdiek/gettitlestring.sty))                  |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./thesis.out)                                                                                 | (./thesis.out)                                                              | (./_minted-thesis/default-pyg-prefix.pygstyle)                 |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ABD:                                                                                           | EveryShipout                                                                | initializing                                                   | macros                                          |                        |                   |                                         |             |                    |                  |          |           |
| *geometry*                                                                                       | driver:                                                                     | auto-detecting                                                 |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| *geometry*                                                                                       | detected                                                                    | driver:                                                        | pdftex                                          |                        |                   |                                         |             |                    |                  |          |           |
| (./thesis.bbl)                                                                                 | (/usr/local/texlive/2016/texmf-dist/tex/latex/amsfonts/umsa.fd)             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| l.243                                                                                          | \end{abstract}                                                              |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| [1]                                                                                            | (./thesis.toc                                                               |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/base/t1cmss.fd))pdfTeX                           | warning                                                                     | (e                                                             |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| (./_minted-thesis/B74DBA7707EAC25E35ADDEA795B45FF900BB0BFBA122918C75635AF8F8D13                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| D7E.pygtex)                                                                                    | [3]                                                                         | [4]                                                            | [5]                                             | [6]                    |                   |                                         |             |                    |                  |          |           |
| (/usr/local/texlive/2016/texmf-dist/tex/latex/base/ts1cmtt.fd)                                 | [7pdfTeX                                                                    | warning                                                        |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (ext4):                                                                                        | destination                                                                 | with                                                           | the                                             | same                   | identifier        | (name{table.1.1})                       | has         | been               |           alread |          |           |
| y                                                                                              | used,                                                                       | duplicate                                                      | ignored                                         |                        |                   |                                         |             |                    |                  |          |           |
| \AtBegShi@Output                                                                               | ...ipout                                                                    | \box                                                           | \AtBeginShipoutBox                              |                        |                   |                                         |             |                    |                  |          |           |
| \fi                                                                                            | \fi                                                                         |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| l.556                                                                                          | U                                                                           |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| nderstandably,                                                                                 | some                                                                        | of                                                             | the                                             | traditional            | constraints       | have                                    | to          | do                 |            with] |          |           |
| [8]                                                                                            | [9]                                                                         |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| Chapter                                                                                        | 2                                                                           |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| (./thesis.gls                                                                                  | [10])                                                                       | [11]                                                           |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| Overfull                                                                                       | \hbox                                                                       | (4.35085pt                                                     | too                                             | wide)                  | in                | paragraph                               | at          | lines              |         672--672 |          |           |
| \T1/cmr/m/it/12                                                                                | Per-for-mant                                                                | Category-Theory                                                | Li-brary                                        | in                     | Coq\T1/cmr/m/n/12 | .                                       | 20          |                    |                  |          |           |
| 14                                                                                             | arXiv:                                                                      | [][]$\T1/cmtt/m/n/12                                           | 1401                                            | .                      | 7694v2$           | [math.CT][][]                           |             |                    |                  |          |           |
| [12]                                                                                           | [13]                                                                        | [14]                                                           | (./thesis.aux)                                  | )                      |                   |                                         |             |                    |                  |          |           |
| (see                                                                                           | the                                                                         | transcript                                                     | file                                            | for                    | additional        | information){/usr/local/texlive/2016/te |             |                    |                  |          |           |
| xmf-dist/fonts/enc/dvips/cm-super/cm-super-t1.enc}{/usr/local/texlive/2016/texm                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| f-dist/fonts/enc/dvips/cm-super/cm-super-ts1.enc}</usr/local/texlive/2016/texmf                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
...
| fonts/type1/public/amsfonts/cm/cmsy10.pfb></usr/local/texlive/2016/texmf-dist/f                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| onts/type1/public/cm-super/sfbx1200.pfb></usr/local/texlive/2016/texmf-dist/fon                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ts/type1/public/cm-super/sfbx1440.pfb></usr/local/texlive/2016/texmf-dist/fonts                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| /type1/public/cm-super/sfbx1728.pfb></usr/local/texlive/2016/texmf-dist/fonts/t                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ype1/public/cm-super/sfbx2488.pfb></usr/local/texlive/2016/texmf-dist/fonts/typ                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| e1/public/cm-super/sfcc1200.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| /public/cm-super/sfit1000.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/p                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ublic/cm-super/sfit1200.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/pub                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| lic/cm-super/sfrm1095.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/publi                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| c/cm-super/sfrm1200.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| cm-super/sfrm1728.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| -super/sfrm2488.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-s                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| uper/sfsx1200.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-sup                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| er/sfsx2488.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-super                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| /sfti1200.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-super/s                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| ftt1000.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-super/sft                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| t1095.pfb></usr/local/texlive/2016/texmf-dist/fonts/type1/public/cm-super/sftt1                |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| 200.pfb>                                                                                       |                                                                             |                                                                |                                                 |                        |                   |                                         |             |                    |                  |          |           |
| Output                                                                                         | written                                                                     | on                                                             | thesis.pdf                                      | (16                    | pages,            | 362872                                  | bytes).     |                    |                  |          |           |
| Transcript                                                                                     | written                                                                     | on                                                             | thesis.log.                                     |                        |                   |                                         |             |                    |                  |          |           |
...
# Example: This cite:agda_overview or \cite{agda_overview} ^_^

#+LaTeX: \addcontentsline{toc}{part}{References}
#+LaTeX: \printbibliography

** COMMENT literal
  1. cite:haskell_modules_formally Diatchki, I. S., Jones, M. P., & Hallgren, T., A formal specification of the Haskell 98 module system , In ,  (pp. 17–28) (). : .
  ...
  4. cite:modular_modules Leroy, X., A modular module system, J. Funct. Program., 10(3), 269–303 (2000).  http://dx.doi.org/10.1017/S0956796800003683

  5. cite:acsl Brito, E., & Pinto, J. S., Program verification in SPARK and ACSL: A comparative case study, In , Reliable Software Technologiey - Ada-Europe 2010, 15th Ada-Europe International Conference on Reliable Software Technologies, Valencia, Spain, June 14-18, 2010. Proceedings (pp. 97–110) (2010). : .
  ...
  8. cite:knuth_lp Knuth, D. E., Literate Programming, Comput. J., 27(2), 97–111 (1984).  http://dx.doi.org/10.1093/comjnl/27.2.97

  9. cite:haskell_backpack Kilpatrick, S., Dreyer, D., Simon L. Peyton Jones, & Marlow, S., Backpack: retrofitting haskell with interfaces, In , The 41st Annual {ACM} {SIGPLAN-SIGACT} Symposium on Principles of Programming Languages, {POPL} '14, San Diego, CA, USA, January 20-21, 2014 (pp. 19–32) (2014). : .
 ...
 13. cite:maude_module_algebra Francisco Dur\'an, & Jos\'e Meseguer, Maude's module algebra, Sci. Comput. Program., 66(2), 125–153 (2007).  http://dx.doi.org/10.1016/j.scico.2006.07.002
 ...
 17. cite:types_for_modules Dreyer, D., Crary, K., & Harper, R., A type system for higher-order modules, In , Conference Record of {POPL} 2003: The 30th {SIGPLAN-SIGACT} Symposium on Principles of Programming Languages, New Orleans, Louisisana, USA, January 15-17, 2003 (pp. 236–249) (2003). : .

 18. cite:dtls_give_modules MacQueen, D. B., Using dependent types to express modular structure, In , Conference Record of the Thirteenth Annual {ACM} Symposium on Principles of Programming Languages, St. Petersburg Beach, Florida, USA, January 1986 (pp. 277–286) (1986). : .

 19. cite:deriving_via Baldur Bl\"ondal, Andres L\"oh, & Scott, R., Deriving via: or, how to turn hand-written instances into an anti-pattern, In , Proceedings of the 11th {ACM} {SIGPLAN} International Symposium on Haskell, Haskell@ICFP 2018, St. Louis, MO, USA, September 27-17, 2018 (pp. 55–67) (2018). : .

 20. cite:hasochism Lindley, S., & McBride, C., Hasochism: the pleasure and pain of dependently typed haskell programming, In , Proceedings of the 2013 {ACM} {SIGPLAN} Symposium on Haskell, Boston, MA, USA, September 23-24, 2013 (pp. 81–92) (2013). : .

 21. cite:10.1145/1411204.1411213 Oury, N., & Swierstra, W., The power of pi, In , Proceedings of the 13th ACM SIGPLAN International Conference on Functional Programming (pp. 39–50) (2008). New York, NY, USA: Association for Computing Machinery.

 22. cite:DBLP:conf/gpce/Al-hassyCK19 Musa Al-hassy, Carette, J., & Kahl, W., A language feature to unbundle data at will (short paper), In I. Schaefer, C. Reichenbach, & T. v. d. Storm, Proceedings of the 18th {ACM} {SIGPLAN} International Conference on Generative Programming: Concepts and Experiences, {GPCE} 2019, Athens, Greece, October 21-22, 2019 (pp. 14–19) (2019). : ACM.

 23. cite:LF_practical_module_system Rabe, F., & Carsten Sch\"urmann, A practical module system for LF, In , Proceedings of the Fourth International Workshop on Logical Frameworks and Meta-Languages: Theory and Practice, {LFMTP} '09, McGill University, Montreal, Canada, August 2, 2009 (pp. 40–48) (2009). : .
 ...
 26. cite:understanding_typescript Bierman, G. M., Mart\'\in Abadi, & Torgersen, M., Understanding Typescript, In , ECOOP} 2014 - Object-Oriented Programming - 28th European Conference, Uppsala, Sweden, July 28 - August 1, 2014. Proceedings (pp. 257–281) (2014). : .

 27. cite:little_theories Farmer, W. M., Guttman, J. D., & Javier Thayer, F., Little theories, In D. Kapur, Automated Deduction---CADE-11 (pp. 567–581) (1992). Berlin, Heidelberg: Springer Berlin Heidelberg.

 28. cite:coq_canonical Gonthier, G., Ziliani, B., Nanevski, A., & Dreyer, D., How to make ad hoc proof automation less ad hoc, J. Funct. Program., 23(4), 357–401 (2013).  http://dx.doi.org/10.1017/S0956796813000051

 29. cite:coq_canonical_tutorial Mahboubi, A., & Tassi, E., Canonical structures for the working coq user, In S. Blazy, C. Paulin, & D. Pichardie, ITP 2013, 4th Conference on Interactive Theorem Proving (pp. 19–34) (2013). Rennes, France: Springer.

 30. cite:wiki_proof_assistants !!! No entry found !!!

 31. cite:agda_overview Bove, A., Dybjer, P., & Norell, U., A brief overview of Agda --- A functional language with dependent types, In , Theorem Proving in Higher Order Logics, 22nd International Conference, TPHOLs 2009, Munich, Germany, August 17--20, 2009. Proceedings (pp. 73–78) (2009). : .

 32. cite:agda_web Jeffrey, A., Dependently typed web client applications - FRP in agda in HTML5, In , Practical Aspects of Declarative Languages - 15th International Symposium, {PADL} 2013, Rome, Italy, January 21-22, 2013. Proceedings (pp. 228–243) (2013). : .
 ...
 40. cite:casl_overview Astesiano, E., Bidoit, M., H\'el\`ene Kirchner, Bernd Krieg-Br\"uckner, Mosses, P. D., Sannella, D., & Tarlecki, A., CASL: the common algebraic specification language, Theor. Comput. Sci., 286(2), 153–196 (2002).  http://dx.doi.org/10.1016/S0304-3975(01)00368-1

 41. cite:idris_main Brady, E. C., IDRIS --- systems programming meets full dependent types, In , Proceedings of the 5th ACM workshop on Programming languages meets program verification (pp. 43–54) (2011). New York, NY, USA: ACM.

 42. cite:idris_website !!! No entry found !!!

 43. cite:idris_faq !!! No entry found !!!

 44. cite:lean_system_desc Leonardo Mendon\cca de Moura, Kong, S., Avigad, J., Doorn, F. v., & Raumer, J. v., The lean theorem prover (system description), In , Automated Deduction - {CADE-25} - 25th International Conference on Automated Deduction, Berlin, Germany, August 1-7, 2015, Proceedings (pp. 378–388) (2015). : .

 45. cite:ats_website !!! No entry found !!!

 46. cite:fstar_website !!! No entry found !!!

 47. cite:beluga Pientka, B., Beluga: programming with dependent types, contextual data, and contexts, In , Functional and Logic Programming, 10th International Symposium, {FLOPS} 2010, Sendai, Japan, April 19-21, 2010. Proceedings (pp. 1–12) (2010). : .

 48. cite:mizar_website !!! No entry found !!!

 49. cite:locales Ballarin, C., Locales and locale expressions in isabelle/isar, In , Types for Proofs and Programs, International Workshop, {TYPES} 2003, Torino, Italy, April 30 - May 4, 2003, Revised Selected Papers (pp. 34–50) (2003). : .

 50. cite:prl_site !!! No entry found !!!

 51. cite:girard_paradox Coquand, T., An analysis of girard's paradox, In , Proceedings of the Symposium on Logic in Computer Science {(LICS} '86), Cambridge, Massachusetts, USA, June 16-18, 1986 (pp. 227–236) (1986). : .
 ...
 55. cite:lf_practical_modules Rabe, F., & Carsten Sch\"urmann, A practical module system for LF, In , Proceedings of the Fourth International Workshop on Logical Frameworks and Meta-Languages: Theory and Practice, {LFMTP} '09, McGill University, Montreal, Canada, August 2, 2009 (pp. 40–48) (2009). : .

 56. cite:matita_main Asperti, A., Coen, C. S., Tassi, E., & Zacchiroli, S., Crafting a proof assistant, In , Types for Proofs and Programs, International Workshop, {TYPES} 2006, Nottingham, UK, April 18-21, 2006, Revised Selected Papers (pp. 18–32) (2006). : .

 57. cite:dtl_imperative Nanevski, A., Morrisett, G., Shinnar, A., Govereau, P., & Birkedal, L., Ynot: dependent types for imperative programs, In , Proceeding of the 13th {ACM} {SIGPLAN} international conference on Functional programming, {ICFP} 2008, Victoria, BC, Canada, September 20-28, 2008 (pp. 229–240) (2008). : .

 58. cite:theories_as_types Dennis M\"uller, Rabe, F., & Kohlhase, M., Theories as types, In , Automated Reasoning - 9th International Joint Conference, {IJCAR} 2018, Held as Part of the Federated Logic Conference, FloC 2018, Oxford, UK, July 14-17, 2018, Proceedings (pp. 575–590) (2018). : .

 59. cite:tpc Carette, J., & O’Connor, R., Theory Presentation Combinators, Intelligent Computer Mathematics, (), 202–215 (2012).  http://dx.doi.org/10.1007/978-3-642-31374-5_14

 60. cite:theory_interpretations_farmer  (Eds.),  (1993). : Springer-Verlag.

 61. cite:mlt_partial Palmgren, E., & Viggo Stoltenberg-Hansen, Domain interpretations of martin-l\"of's partial type theory, Ann. Pure Appl. Logic, 48(2), 135–196 (1990).  http://dx.doi.org/10.1016/0168-0072(90)90044-3

 62. cite:typeclasses_for_maths Spitters, B., & Weegen, E. v. d., Type classes for mathematics in type theory, Mathematical Structures in Computer Science, 21(4), 795–825 (2011).  http://dx.doi.org/10.1017/S0960129511000119
 ...
 69. cite:ambiguous_parsing Coen, C. S., & Zacchiroli, S., Efficient ambiguous parsing of mathematical formulae, Mathematical Knowledge Management, (), 347–362 (2004).  http://dx.doi.org/10.1007/978-3-540-27818-4_25
 ...
 75. cite:DBLP:conf/hopl/HudakHJW07 Hudak, P., Hughes, J., Simon L. Peyton Jones, & Wadler, P., A history of Haskell: being lazy with class, In B. G. Ryder, & B. Hailpern, Proceedings of the Third {ACM} {SIGPLAN} History of Programming Languages Conference (HOPL-III), San Diego, California, USA, 9-10 June 2007 (pp. 1–55) (2007). : ACM.

 76. cite:data_monoid !!! No entry found !!!

 77. cite:DBLP:journals/jfp/Swierstra08 Swierstra, W., Data types \`a la carte, J. Funct. Program., 18(4), 423–436 (2008).  http://dx.doi.org/10.1017/S0956796808006758
 ...
 82. cite:lof_constructive_math Martin-L\"of, P., Constructive mathematics and computer programming, In , Proc. Of a Discussion Meeting of the Royal Society of London on Mathematical Logic and Programming Languages (pp. 167–184) (1985). Upper Saddle River, NJ, USA: Prentice-Hall, Inc.

 83. cite:coq_website !!! No entry found !!!

 84. cite:coq_inductive_coc Pfenning, F., & Christine Paulin-Mohring, Inductively defined types in the calculus of constructions, In , Mathematical Foundations of Programming Semantics, 5th International Conference, Tulane University, New Orleans, Louisiana, USA, March 29 - April 1, 1989, Proceedings (pp. 209–228) (1989). : .

 85. cite:tactics Kamal Aboul-Hosn, A proof-theoretic approach to tactics, In , Mathematical Knowledge Management, 5th International Conference, {MKM} 2006, Wokingham, UK, August 11-12, 2006, Proceedings (pp. 54–66) (2006). : .
 ...
 88. cite:seven_hundred_langs Landin, P. J., The next 700 programming languages, Commun. {ACM, 9(3), 157–166 (1966).  http://dx.doi.org/10.1145/365230.365257

 89. cite:seven_hundred_tt_models Boulier, S., Pierre-Marie P\'edrot, & Tabareau, N., The next 700 syntactical models of type theory, In , Proceedings of the 6th {ACM} {SIGPLAN} Conference on Certified Programs and Proofs, {CPP} 2017, Paris, France, January 16-17, 2017 (pp. 182–194) (2017). : .

 90. cite:seven_hundred_provers Paulson, L. C., Isabelle: the next 700 theorem provers, CoRR, cs.LO/9301106(),  (1993).

 91. cite:seven_hundred_hoas Felty, A. P., Momigliano, A., & Pientka, B., The next 700 challenge problems for reasoning with higher-order abstract syntax representations - part 2 - A survey, J. Autom. Reasoning, 55(4), 307–372 (2015).  http://dx.doi.org/10.1007/s10817-015-9327-3
 ...
 93. cite:seven_hundred_data Fisher, K., Mandelbaum, Y., & Walker, D., The next 700 data description languages, J. {ACM, 57(2), 10–1–10–51 (2010).  http://dx.doi.org/10.1145/1667053.1667059
