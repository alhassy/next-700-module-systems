...
- [[#abstract][Abstract]]
- [[#introduction----the-thesis-story][Introduction ---The Thesis’ “Story”]]
  - [[#intro][Intro]]
...
- TODO [[#current-approaches][Current Approaches]]
  - [[#intro-1][Intro]]
  - [[#expectations-of-module-systems][Expectations of Module Systems]]
  - [[#ad-hoc-grouping-mechanisms][Ad hoc Grouping Mechanisms]]
  - [[#existing-systems][Existing Systems]]
  - [[#facets-of-structuring-mechanisms-an-agda-rendition][Facets of Structuring Mechanisms: An Agda Rendition]]
...
- TODO [[#packageformer][PackageFormer]]
  - [[#syntax][Syntax]]
  - [[#extension][Extension]]
  - [[#defining-a-concept-only-once][Defining a Concept Only Once]]
  - [[#renaming-1][Renaming]]
  - [[#union-and-intersection][Union (and intersection)]]
  - [[#duality][Duality]]
  - [[#extracting-little-theories][Extracting Little Theories]]
  - [[#200-theories----one-line-for-each][200+ theories -- one line for each]]
  - [[#summary-of-sample-variationals-provided-with-the-system][Summary of Sample Variationals Provided With The System]]
- TODO [[#contexts][Contexts]]
  - [[#introduction-1][Introduction]]
  - [[#the-problems][The Problems]]
  - [[#monadic-notation][Monadic Notation]]
  - [[#termtypes-as-fixed-points][Termtypes as Fixed-points]]
  - [[#free-datatypes-from-theories][Free Datatypes from Theories]]
...
+ *§ 4 Module Meta-primitives as Library Methods*
...
+ *§ 5 Conclusion: The lingua franca dream as reality*
...
mechanism”, how it can be simulated when it is not a primitive construct, how
Agda handles them, and a brief theory of their foundations.
#+end_quote
...
[3] *Sub-Modules:* If a module is encoded as a record, then a sub-module is a field in
the record which itself happens to be a module encoding.

[4] *Parameterised Modules:* If a module can be considered as encoded as the returned
record from a function, then the arguments to such a function are the parameters
to the module.

[5] *Mixins:* A /mixin/ is the ability to extend a datatype /X/ with functionality /Y/ long
after, and far from, its definition. Mixins ‘mix in’ new functionality by
...
less than 15 /actively developed/ dependently-typed languages in-use /that are also
used/ as proof-assistants ---which are intersting to us since we aim to mechanise
all of our results: Algorithms as well as theorems.

*Agda cite:agda_overview,agda_thesis*: One of the more popular proof assistants
...
modules which may contain sub-modules along with declarations and definitions of
first-class citizens. The intimate relationship between records and modules is
perhaps best exemplified here since the current implementation provides a
...
functional parameter; as such, a computationally expensive parameter provided to
a module invocation may be intended to be computed only once, but is actually
computed at each call site.
...
module system. A notable difference from Agda is that it allows “copy and paste”
...
cite:coq_manual. Since Coq module types are essentially contexts, the module
type ~X <+ Y <+ Z~ is really the catenation of contexts, where later items may
depend on former items. The Maude cite:maude,maude_module_algebra framework
contains a similar yet more comprehensive algebra of modules and how they work
with Maude theories. The Common Algebraic Specification Language
...
rather than theorem proving” cite:idris_faq, idris_tdd.
...
i.e., for each 𝒮-symbol $f$ of arity $n$, {{{newline}}}
