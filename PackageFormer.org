...
(cl-defun substring-delimited
  ...
  (let (longest-needle context first-ending result)
    ...
    (unless preserve-spaces (setq string (s-collapse-whitespace (or string ""))))
...
( Lisp convention would advise this function to be named ~with-pf~, but I'm using the name ~open~,
...
#RESULTS:
| #s(package-former nil PackageFormer Semigroup  1 4 (v : Variation PackageFormer v _⨾_ : Semigroup v → Semigroup v → Semigroup v Id  : Semigroup v assoc : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z))) |
...
(cl-defun show-package-former (p &key (omit-field-decl t) omit-docstring omit-car-element)
  ...
  (open-pf p (concat
     ...
     ;; are maked as singelton lists and their indentation is lessened.
...
(defun load-instance-declaration (line)
  ...
  (let* (inst (pieces (s-split " " line)) (more (cddddr pieces)) renames hidings exposes manifest (unbundling 0))
...
#+RESULTS:
: #s(instance-declaration "MagmaR = Magma record unbundling 3" "MagmaR" "Magma" "record" 3 nil nil nil nil)
  ...
  (if (equal 700-comments 700-cmnts) (message "700-comments Unchanged.")
    ...
    (while lines
     ...
     (if (load-instance-declaration item) (setq lines (cdr lines))
       ...
       (add-to-list 'package-formers (parse-package-former (cadr item))) ;; acknowledge PackageFormer declaration, if any
...
PackageFormer Monoid (v : Variation) : Set (ℓ.zero) where
    _⨾_     : Monoid v → Monoid v → Monoid v
    Id      : Monoid v
    assoc   : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)
    leftId  : ∀ {x : Carrier} → Id ⨾ x ≡ x
...
(#s(package-former nil "PackageFormer" "Monoid" "(ℓ.zero)" 1 4
                   ("v : Variation" "Monoid v : Set(ℓ.zero)" "_⨾_     : Monoid v → Monoid v → Monoid v" "Id      : Monoid v" "assoc   : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftId  : ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightId : ∀ {x : Carrier} → x ⨾ Id ≡ x")))


(#s(instance-declaration "MonoidB = Monoid record with (Carrier to Bool; Id to false)" "MonoidB" "Monoid" "record" -1 nil nil
                         (("Carrier" . "Bool")
                          ("Id" . "false"))
                         nil)
   #s(instance-declaration "MonoidE = Monoid record exposing (Carrier; Id)" "MonoidE" "Monoid" "record" 2 nil nil nil
                           ("Carrier" "Id"))
   #s(instance-declaration "MonoidR = Monoid record unbundling 2" "MonoidR" "Monoid" "record" 2 nil nil nil nil)
  ...
  (setf (package-former-elements pf)
        (-concat
             prefix-fields
               ...
               (funcall alter-fields (-filter keep-fields (package-former-elements pf)))
             suffix-fields
           ))
...
(instantiate-as-unbundled 1 pfs  :waist (instance-declaration-waist inst) :alter-fields (lambda (fs)
                                                         (funcall exposes-hoisting ;; for exposes variational clause
...
{- This was generated from the PackageFormer Semigroup. -}
record Semigroup- (Carrier : Set(ℓ.suc ℓ.zero)) (Id : Carrier) : Set (ℓ.suc(ℓ.suc ℓ.zero)) where
  field
    _⨾_ : Carrier → Carrier → Carrier
    assoc : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)
    leftId : ∀{x} → Id ⨾ x ≡ x
