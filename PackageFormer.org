...
*** Variationals with Arguments ---~map, rename, [co]decorated~
...
*** TODO COMMENT Generalising ~primed~
   ...
   preprended with ~let x = x′ in~ to propogate this change ---the clause is necessary
   since it may be difficult in propogating mixfix names.
...
{-lisp
(eval (𝒱 primer = :alter-elements (lambda (fs)
  (-let [fsnew fs]
  (loop for old in fs
        for new in (--map (map-name (concat name "′") it) fs)
  do (setq fsnew (--map (s-replace old new it) fsnew))))))
-}
...
*** TODO Primed Decoration, Composition ~⟴~, and First-class PackageFormers :outdated:
...
*** TODO Forming Syntax and the Special ~$𝑛𝑎𝑚𝑒~ Variable :outdated:
...
** Future Instantiations Remaining
...
;; MA: TODO: Allow {-lisp -} comments so that code that is long, as follows
;; needn't be squezzed into one absurdly long line.
...
;; the above rendered as 𝒱 function.
(cl-defun 𝒱-generated (&key by *parent-context*)
  " See PF.org"
  (lambda (fs)
...
; (load-instance-declaration "M₁  =  M-Set ⟴ primed ⟴ data" 'show-me)
; (load-instance-declaration "M₂  =  M-Set ⟴ data ⟴ primed" 'show-me)

;; (load-instance-declaration "ScalarD₀ = M-Set data" 'show-me)
;; ; (load-instance-declaration "ScalarD  = ScalarD₀ data ⟴ rename :elements (lambda (name) (concat name \"D\"))" 'show-me)
;; (load-instance-declaration "ScalarD  = ScalarD₀ map :elements (lambda (e) (map-name (concat name \"g\") e))" 'show-me)
;; ; (pp package-formers)
; (load-instance-declaration "ScalarD  = M-Set primed" 'show-me)
(load-instance-declaration "Mₑ  =  M-Set" 'show-me)
#+END_SRC

#+RESULTS:
#+begin_src results-agda
{- Mₑ  =  M-Set -}
PackageFormer Mₑ : Set₁ where
   Scalar  : Set
   Vector  : Set
   _·_     : Scalar → Vector → Vector
   𝟙       : Scalar
