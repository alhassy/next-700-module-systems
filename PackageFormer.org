...
*** TODO COMMENT generated-by / keeping
...
(funcall
(lambda (fs)
  (let* ( fsnew (p (lambda (f) (equal "leftId" (get-name f))))
               (yeses (--map (funcall p it) fs))
               (get-yeses (lambda () (--filter it (--zip-with (if it other) yeses fs))))
               (in-yeses (lambda (e) (--any (s-contains? (s-replace "_" " " (get-name e)) (get-type it)) (funcall get-yeses)))))
...
(package-former-elements (cdr (assoc "M-Set" package-formers)))))
#+END_SRC
