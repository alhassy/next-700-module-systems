...
⟪VariationalClause⟫ ::= ⟪VariationalClause⟫ ⟴ ⟪VariationalClause⟫
...
+ One derives many presentations of a grouping mechanism by what we call ‘variational clauses’.
  - In a multiline comment, a non-700-comment, one declares ‘varitionals’ such as
     ...
     and their right hand side may invoke any of the 4 meta-primitives
...
** List of  ~instantiations-remaining~
...
(load-package-former (second (get-children "PackageFormer" test)))
(let ((mid (load-instance-declaration "NewName = M-Set test :kind (module) 𝓌𝒾𝓉𝒽 :waist (2) ⟴ test′ :height (1)")))
...
#+RESULTS:
#+BEGIN_src results-agda
{- NewName = M-Set test :kind (module) 𝓌𝒾𝓉𝒽 :waist (2) ⟴ test′ :height (1) -}
module NewName (Scalar : Set) : Set₁ where
   Vector  : Set
   _·_     : Scalar → Vector → Vector
   𝟙       : Scalar
   _×_     : Scalar → Scalar → Scalar
...
(load-package-former (second (get-children "PackageFormer" test)))
(let ((mid (load-instance-declaration "NewNasme = M-Set primed-record")))
...
#+RESULTS:
#+BEGIN_src results-agda
{- NewNasme = M-Set primed-record -}
record NewNasme : Set₁ where
 field
   Scalar′ : Set
   Vector′ : Set
   _·′_ : Scalar → Vector → Vector
