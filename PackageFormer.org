...
---e.g., homomorphisms, syntax, interpreation functions---
...
| ~Testing.agda~ has up-to-date working examples. |
...
we can currently be done. The acutaly syntax that is currently supported by this
program is suprinsgly close to the above, with an occasional ~-~ or ~:~ inserted.
...
#+BEGIN_SRC text
 700: This kind “recorder” is not supported by Agda!
    ...
    ⇨	whoops  = :type recorder :waist-strings '("field")
#+END_SRC
  ...
  as they should be ---including the *important* new lines.
...
#+BEGIN_src results-agda
{- 𝒱-primed-attempt  = :type record :waist-strings '("field") :alter-elements (lambda(f) (map-name (concat name \"′\") f))
...
#+END_src
   ...
   is not an admissiable construct in Agda.

   Moreover, if we try to instantiate this package further, say as in
   ~M′ = M-Set′-attempt record~ we will have fields such as ~𝟙′ : Scalar~
   whose type is a free variable: ~Scalar~ no longer refers to any field!

   The solution then is to /propogate/ any changes a name has down to its siblings.

** Composition ~⟴~ and First-class PackageFormers

    Before we show how to fix the flawed ~primed-attempt~ definition,
    it is important to note that the previous variational duplicated
    the ~record~'s definition in full.
...
carrier name with the name of the newely defined PackageFormer ---which we may access
using the special identifier ~$𝑛𝑎𝑚𝑒~.
#+BEGIN_src agda
 {-lisp
(𝒱 data carrier
  = :type data
...
** Variationals with Arguments ---~map, rename, [co]decorated, renaming~
...
#+END_src
   ...
   Let's turn to a slightly more complicated example: Generalising ~primed~ to take arguments. Whenever a name ~x~ is primed, all subsequent elements' type should be
   preprended with ~let x = x′ in~ to propogate this change.
...
#+END_SRC
  ...
  ~unzip, map, zip-with~. Anyhow, here is a better approach to doing essentially the
  same thing; for diversity, the following scheme will not produce ~let…in…~ clauses
...
1. The Lisp code now lives in a ~{-lisp ⋯ -}~ block.

2. 700-comments have single-line ~𝒱-name args = rhs~, whereas lisp-blocks
   have multi-line ~(𝒱 name args = rhs)~ ---the dash after the 𝒱 is gone and outter-most
   parenthesis are added.

3. To provide minimal accomodation for mixfix names, we simply remove the
   Agda argument indicator ‘_’ when performing rewrites.

   Let's code this up as a reusable pattern ---call it ~map~.
#+BEGIN_SRC agda
{-lisp
...
and show how using ~progn~ allows mutltiple forms to occur.
#+BEGIN_SRC agda
{-lisp
(progn
...
Example ~M₁₋₂~ demonstrates that composition, ⟴, is sequential from left to right.
That is, “⟴” is just forwards composition: We thread the given PacakgeFormer
through the variationals ~vᵢ~ in order. Operationally:

| Pf v₀ ⟴ ⋯ ⟴ vₙ ≈ ((Pf v₀) v₁) ⋯) vₙ |
| Pf ⟴ v  ≈  Pf v ⟴  ≈  Pf v          |

Note: In the concrete syntax, parenthisation is not permitted.

What if we want Agda-style renaming via ~to~-lists?
...
** Shallow Renaming with Agda's ~open ⋯ public ⋯ renaming ⋯~
    The previous approach to renaming altered field names literly which is not
...
#+END_SRC
   ...
   One-hundred: A one-paremeter module where elements may be renamed.
#+BEGIN_SRC agda
{-lisp
...
** Subpacakges with ~generated, sorts, signature~
    ...
    pacakge that contains only certain specfiied elements. For example,
...
#+END_SRC
    ...
    For clarity, rather than efficieny,
...
We can even obtain a sub-signature wholsesale:
#+BEGIN_SRC results-agda
{-700
...
This pattern of having a lawless grouping seems sufficinetly desirable that we may
codify it.
#+BEGIN_SRC results-agda
{-lisp
...
** Automatically deriving homomorphism definitions ♥‿♥
    ...
    :Illumniating_yet_overkill_to-subscript_defn:
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
    ...
    Then, we form the variational as follows ---alos in a ~lisp~ block.
#+BEGIN_SRC emacs-lisp
(𝒱 hom
  = record ⟴
    :waist 2
...
#+END_SRC
    ...
    wish ---which may be preferable to extending the variational to accomodate for new
    names.
...
#+END_SRC
  ...
  Homomorpism equality, application to carrier elements, isomorphisms,
  ...
  + Challenge ::
    ...
    4. Write a Lisp code that produces a function ~MakeProduct : (A₀ A₁ : M) → Prod A₀ A₁~.
       ...
       - The projection morphisms are straightfoward.
...
** TODO COMMENT List of derivied variationals that are common and useful :outdated:
...
+ ~rename-mixfix~ :: Renamaing Agda mixifix names where the rename operation
     ignores the outermost position markers ‘_’.
...
Let's try this out.
#+BEGIN_SRC emacs-lisp :exports both :results table replace
...
* TODO  COMMENT Orphan content :important:noexport:
...
(load-variational "𝒱-primed-record = :type 'module ⟴ :level 'inc :alter-elements (lambda (f) (map-name (concat name \"′\") f))")
(setq package-formers nil)
(load-package-former (second (get-children "PackageFormer" test)))
(load-instance-declaration "NewNamfe = M-Set primed-record ⟴ :type 'record :waist 2 :waist-strings (\"field\")")
#+END_SRC
...
(cl-defun instantiate (id)
  ...
  (letf ((self (copy-package-former (cdr (assoc (instance-declaration-package-former id) package-formers))))
    ...
    (unless self (error (format "700: %s\n\n\t⇨\t%s\n\t⇨\t%s ≈ %s"
      ...
      ;; :kind ≈ The vocabulary that replaces “PackageFormer”.
      (⟰ 'type '(-contains? '(record data module PackageFormer) it)
 ...
 #+BEGIN_SRC emacs-lisp :var test = test :exports both :results replace :wrap "src results-agda"
;   "𝒱-primed-record = :kind record :waist-strings (\"field\")
(load-variational "𝒱-primed-record = :type record ⟴ :level nice :alter-elements (lambda (f) (map-name (concat name \"′\") f))")
(setq package-formers nil)
(load-package-former (second (get-children "PackageFormer" test)))
...
* COMMENT Cool stuffs, but now obsolote -e.g., :itify:
** itify
