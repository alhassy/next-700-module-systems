...
#+END_SRC
  ...
  doing so otherwise may override existing utilities in Emacs
...
Another test:
TODO: Erroneous output!
#+BEGIN_SRC emacs-lisp :var test = test :exports both :results replace :wrap "src results-agda"
; (load-variational "𝒱-tc height lvl = :type 'record :waist-strings '(\"field\") :waist height :level lvl")
; (load-variational "𝒱-tc height lvl = :level 'inc :waist height :level lvl")
(load-variational "𝒱-tc lvl = :level lvl")
(setq package-formers nil)
(load-package-former (second (get-children "PackageFormer" test)))
(load-instance-declaration "LHS = M-Set :level dec ⟴ tc :lvl 'inc" 'show-me)
#+END_SRC

#+RESULTS:
#+begin_src results-agda
{- LHS = M-Set :level dec ⟴ tc :lvl 'inc -}
PackageFormer LHS : Set₂ where
   Scalar  : Set
   Vector  : Set
   _·_     : Scalar → Vector → Vector
   𝟙       : Scalar
...
#+BEGIN_SRC emacs-lisp :var test = test :exports both :results replace :wrap "src results-agda"
(load-variational "𝒱-primed-record = :type record")
...
** COMMENT ~instantiate~ ---the core utility
...
* Old, possibly no longer needed, code fragments
...
* PackageFormers *as* Agda Datatypes      :Eventually::after_prototype_is_done:
