...
If the previous section is unclear at the aims and uses of this prototype, please consult
the pre-print [[https://github.com/alhassy/next-700-module-systems-proposal/blob/master/Paper0.pdf][A Language Feature to Unbundle Data at Will]] or [[https://alhassy.github.io/next-700-module-systems-proposal/][the next 700 module systems proposal]].
...
0. A menu-bar ~700PackageFormers~ will now be enabled.
...
;;
;; Dually, thread-last behaves the same but being at the final-most form position.
...
(cl-defun extract-imports ()
  "Return substring of buffer whose lines mention “import”.
   Throw away any that mention the substring “_Generated”.
  "

  ;; TODO: Remove old code
  ;; (s-join "\n" (--remove (s-contains? "_Generated" it) (--filter (s-contains? "import" it) (s-split "\n" (buffer-substring-no-properties (point-min) (point-max))))))

  (thread-last (buffer-substring-no-properties (point-min) (point-max))
    (s-split "\n")
    (--filter (s-contains? "import" it))
    (--remove (s-contains? "_Generated" it))
    (s-join "\n")
  )
)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
;; MA: TODO: Discard this item? c.f. “3.” below.
(defun special (f)
  (--any? (s-contains? it f) '("field" "private")))

(cl-defun show-package-former (p &key (omit-field-decl t) (omit-level nil) omit-docstring omit-car-element)
  ...
  (open-pf p (concat
     ...
     (unless omit-field-decl
       (concat (s-repeat (- indentation 2) " ") "field\n"))

     ;; TODO: Relaise the field keyword as a special field, cf 3 below.

     ;; 3. The fields
     ;; “Special fields”, for whatever reason are exceptional, and so
...
;;        (--map
;;         (pcase (if (consp it) (list (car it) 2) (list it 0))
;;           (`(,f ,i) (concat (s-repeat (- indentation i) " ") f))) fields)))
       (s-join "\n"
       (funcall (if omit-car-element #'cdr #'identity)
       (--map (concat (s-repeat (- indentation (if (special it) 2 0)) " ") it) fields)))
    )
  )
)
#+END_SRC
...
(-let [pf (cadr (get-children "PackageFormer" test))]
   (show-package-former (parse-package-former pf) :omit-docstring t :omit-car-element t))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :var test = test :exports both :results table replace
(-let [pf (cadr (get-children "PackageFormer" test))]
  (equal (s-join "\n" pf) (show-package-former (parse-package-former pf) :omit-docstring t :omit-car-element t)))
#+END_SRC
...
(defun load-instance-declaration (line)
  ...
  (let* (inst (pieces (s-split " " (s-collapse-whitespace line))) (more (cddddr pieces)) renames hidings exposes manifest (unbundling 0) decorations openings openings-param)
   (when (and (<= 4 (length pieces)) (equal (nth 1 pieces) "="))
     ...
     ;;
     ;; NOTE: This clause differs from the rest: No variation tag ‘record, data, typeclass’.
     ;; Since a ⟪record-name⟫ is requested, even the PF is irrelevant.
...
(ert-deftest load-inst () ""
  :expected-result :failed
  (should (equal (load-instance-declaration "MagmaR = Magma record renaming (Carrier to C; _⨾_ to _∘_)")
                 #s(instance-declaration "MagmaR = Magma record renaming (Carrier to C; _⨾_ to _∘_)" "MagmaR" "Magma" "record" 0
                 (("Carrier" . "C") ("_⨾_" . "_∘_")) nil nil nil nil nil)))
...
(load-instance-declaration
  "LHS = Magma opening (MagmaR; Carrier to C; _⨾_ to _∘_; it ++ \"′\"") ;; MA: TODO: Acknowledge init-tail of to-list.
#+END_SRC

#+RESULTS:
: #s(instance-declaration "LHS = Magma opening (MagmaR; Carrier to C; _⨾_ to _∘_; it ++ \"′\"" "LHS" "Magma" "opening" 0 nil nil nil nil nil "(concat  it   \"′\")" "MagmaR")

Testing renames:
#+BEGIN_SRC emacs-lisp :results replace
(pp (load-instance-declaration
  " = Magma record renaming (Carrier to C; _⨾_ to _∘_)"))
#+END_SRC

#+RESULTS:
: #s(instance-declaration " = Magma record renaming (Carrier to C; _⨾_ to _∘_)" "" "Magma" "record" 0
: (("Carrier" . "C")
: ("_⨾_" . "_∘_"))
...
#+BEGIN_SRC emacs-lisp :results replace
(pp (load-instance-declaration
  "MonoidB = Monoid record with (Carrier to 𝔹; Id to false)"))
#+END_SRC

#+RESULTS:
: #s(instance-declaration "MonoidB = Monoid record with (Carrier to 𝔹; Id to false)" "MonoidB" "Monoid" "record" -1 nil nil
: (("Carrier" . "𝔹")
: ("Id" . "false"))
: nil)
...
#+BEGIN_SRC emacs-lisp :results replace
(pp (load-instance-declaration
  "MonoidB = Monoid record decorated (it ++ \"′\")"))
#+END_SRC

#+RESULTS:
: #s(instance-declaration "MonoidB = Monoid record decorated (it ++ \"′\")" "MonoidB" "Monoid" "record" 0 nil nil nil nil "(concat it   \"′\")")
...
(cl-defun instantiate (decls &key
  ...
  "Given a PackageFormer declaration, instantiate it into a concrete Agda type.
   ...
   - ‘has-field-decl’: Is the keyword “field” to appear?
  ...
  (setf (package-former-docstring pf)
    (or docstring (concat "This was generated from the PackageFormer " pf-name ".")))
  ...
  (when (and (equal waist 0) inc-level)
    (setf (package-former-level pf) (concat "(Level.suc" (if (s-blank? (package-former-level pf)) " Level.zero" (package-former-level pf)) ")")))
  ...
  ;; 6. Stringify!
  (show-package-former pf :omit-field-decl (not has-field-decl)
...
(cl-defun instantiate-as-data (decls &key new-name (carrier "Carrier") docstring (alter-fields #'identity) (waist 0))
  ...
  (let* ((pf (if (package-former-p decls) (copy-package-former decls) (parse-package-former (cadr (get-children "PackageFormer" decls)))))
    ...
    ;; TODO: MA: Should this be ‘decls’ or ‘pf’? Write some tests.
...
(defun get-type (tn)
  "Given a string “name : type”, return the ‘type’.
  Whitespace at the edges is trimmed away."
...
(assert (equal (get-type"Id : ∀ {X : Obj 𝒞} → (X ⟶ X)") "∀ {X : Obj 𝒞} → (X ⟶ X)"))
#+END_SRC
    ...
    (instantiate (car package-formers)
      ...
      ;; :alter-fields (lambda (fs) fs)
                 ...
                 (when nil :new-name new-name
                    ...
                    ;; opening formation
                    (let (fsnew)
                    ; (loop for f in (funcall alter-fields (cdr fs)) ;; omit ‘carrier’
...
(instantiate-as-opening (car package-formers) :new-name "Decorated₁")
#+END_SRC
...
#+end_src
  ...
  (let* () ; (pf (if (package-former-p decls) (copy-package-former decls) (parse-package-former (cadr (get-children "PackageFormer" decls)))))
    ...
    (instantiate decls    :new-name "Semantics₁!!!"
                    :type "module"
                    ; :docstring "???"
                    :has-field-decl nil
                    :omit-level-decl t
...
Let's test it out.
#+BEGIN_SRC emacs-lisp :var test = test :exports both :results replace :wrap "src results-agda"
(instantiate-as-opening "Monoid′  = Monoid opening (MonoidRDT; it ++ \"′\")")
#+END_SRC
...
{- MonoidTypeclass = Monoid typeclass hiding (_⨾_) -}
record MonoidTypeclass (Carrier : Set(ℓ₀)) : Set (ℓ₀) where
  field
    Id      : Carrier

{- MonoidT = Monoid typeclass renaming (Carrier to C; _⨾_ to _⊕_) -}
record MonoidT (C : Set(ℓ₀)) : Set (ℓ₀) where
  field
...
{- MonoidR = Monoid record unbundling 2 -}
record MonoidR (Carrier : Set(ℓ₀)) (_⨾_ : Carrier → Carrier → Carrier) : Set (ℓ₀) where
  field
    Id      : Carrier
    assoc   : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)
    leftId  : ∀ {x : Carrier} → Id ⨾ x ≡ x
...
record MonoidE (Carrier : Set(ℓ₀)) (Id : Carrier) : Set (ℓ₀) where
  field
    _⨾_     : Carrier → Carrier → Carrier
    assoc   : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)
    leftId  : ∀ {x : Carrier} → Id ⨾ x ≡ x
...
;; ⇒ (defalias (quote ap-it) (cons (quote macro) (function (lambda (itbody more) (list (quote ap) (list (quote lambda) (quote (it)) itbody) more)))))
...
;;
(assert (equal (--map (rename-mixfix-it (format "₀%s¹" it) it) '("_⊕_" "_[_⊗_]" "he_lo" "he-lo"))
       ;; Outermost ‘it’ belongs to --map; inner-most ‘it’ belongs to rename-mixfix-it.
  '("_₀⊕¹_" "_₀[_⊗_]¹" "₀he_lo¹" "₀he-lo¹")))

(rename-mixfix #'identity "_⊕_")

(rename-mixfix-it (eval (car (read-from-string "(concat it \"′\")"))) "_⊕_") ;; ⇒ _⊕′_
#+END_SRC
...
(setq opening-formation (lambda (fs) (list "opening MonoidR : 𝒩yet")))
...
;; instantiate-as-unbundled 0 pfs  :waist (instance-declaration-waist inst) :alter-fields …
