...
2. $h₂$ is a semigroup homomorphism
3. $h₁ (m · a) ≈ h₁ m ·′ h₂ a$, a reasonable coherence condition.

Theorems:
1. (Id, Id) is a MA-Set homomorphism for any (ℳ, 𝒜)-set.
   - Sketch: Id is a monoid and semigroup homomorphism, and the coherence
     condition is true by ≈-reflexivity.
2. MA-Set homomorphisms are closed under composition.
   - Sketch: The composition of monoid/semigroup homomorphisms is again
