...
a fairly economical way [TODO:citation:???]. The union operation is an instance of a /pushout/
operation, which consists of 5 arguments ---three objects and two morphisms--- which may
be included into the ~union~ operation as optional keyword arguments, which we will return to
in the second part of the user manual. The more general notion of pushout is required if
we were to combine ~GroupR~ with ~AbealianMonoidR~, which have non-identical syntactic
copies of ~MonoidR~.

{{{fold(Definition of union -- Feel free to ignore until later)}}}
#+BEGIN_SRC emacs-lisp :tangle "variationals.tmp" :noweb-ref std-𝒱-lib :noweb yes
(𝒱 union pf
...
The ~extended-by~ variational allows Agda users to easily employ the /tiny theories/ [little_theories,mathscheme]
...
** Subpacakges with ~generated, sorts, signature~
   :PROPERTIES:
...
** Sharoda's “Leveraging the information contained in theory presentations”
   :PROPERTIES:
   :CUSTOM_ID: Sharoda-s--Leveraging-the-information-contained-in-theory-presentations-
   :END:

   In her PhD proposal, pages 1-3, she shows how an explicit definition of monoid can mechanically gives rise
   to:
   + [X] signatures
   + [X] termtypes
     - Covered in our gpce_19 paper.
   + [X] open termtypes, those with variables
     - Covered in our gpce_19 paper.
   ...
   She proposes that universal algebra ---a uniform approach to studying different algebras---
   is currently performed on-demand and manually in computing, which it needn't be.
