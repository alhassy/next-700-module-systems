...
;; Below “meta-variables” are quoted...?
(𝒱 for x from xs let res become body where local is definition
 = "noice
...
As usual, if DEFINITION is a string, it cannot contain the ‘\’ character;
which is sidestepped via the Unicode character ‘╲’.
...
_ = declare for x in (𝟙 𝟚 𝟛) let (format \"CarrierU%s\" x) = (Carrier renaming \"U to 𝒰\") where 𝒰 = (format \"U%s\" x)

Terse as in Haskell:
_ = declare for x ∈ (𝟙 𝟚 𝟛) ∷ (format \"CarrierU%s\" x) = (Carrier renaming \"U to 𝒰\")  ⇐  𝒰 = (format \"U%s\" x)

It is important to remember that “declare” is not a builtin keyword, it is the name of a PackageFormer
---we have chosen to name the empty PackageFormer “declare” as it makes the resulting invocation readable.
The LHS “_” means that this invocation is used for its side-effects, to produce new PackageFormers,
rather than for producing a PackageFormer by altering the given one, “declare”.

The LHS could be given any name which then serves to *document* the purpose of the loop;
e.g., “CarrierU𝒾⟵𝒾∶1…3” for the above clauses. This is cool: WE HAVE NAMED DECLARATIONS.
"
   :alter-elements (λ _ →

                      (loop for x in xs
                            for local = (regexp-quote (format "%s" 'local)) ;; The quote is in-case the variable name is a regexp operator such as +,?,*, etc.
                            for value = (s-replace "" (regexp-quote "") definition) ;; The quote is in-case the definition results in a regexp callback, such as \1, \\𝒾, etc.
                            ;; A variable may, ‘?’, have a quote before or after it;
                            ;; Which we reproduce in the resulting string via ‘\\𝒾’, for each match 𝒾.
                            ;; Otherwise, it is a whole word; as delimited by ‘\\b’.
...
;; (profiler-stop) ⇒ Only defined whence profiler start called.
(profiler-start 'cpu) (profiler-stop)

;; 𝒱-exposing
;; Go    = MonoidP :alter-elements (λ es → (progn (setq _expose (length (--graph-map (funcall (reify-to-list "Id; _⨾_" :otherwise nil :key #'element-name) it) it es))) es)) ⟴ :waist _expose
#+END_SRC
...
ORIG-FUN is intended to be the Agda loading process with arguments ARGS."
  ...
  (-let [counter 0] ; agda2-in-progress
    ...
    ;; ⇦ ⇦ ⇦ ⇦ ⇦  ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦ ⇦
    (message "PackageFormer (•̀ᴗ•́)و All the best coding! %.06f" (float-time (time-since start-time)))))
#+END_SRC
:End:

:Issue:
{-700
PackageFormer RightMonoidAction : Set₁ where
    e     : U
    identity_⟫_e : U
    toX : let View X = X in View toX ; toX = record {U = U;_*_ = _*_;e = e;S = S;_⟫_ = _⟫_;identity_⟫_e = identity_⟫_e}
    toRightUnitalAction     : let View X = X in View RightUnitalAction ;    toRightUnitalAction = record {U = U;_*_ = _*_;e = e;S = S;_⟫_ = _⟫_;identity_⟫_e = identity_⟫_e}

RightMonoid𝟙Action = RightMonoidAction renaming′ "e to 𝟙"
-}
:End:
...
#
# ~# extended-by ds~
# ⟴ Allow ‘ds’ to be a list of elements!
# ⟴ Whence, ~extended-by (elements-of pf)~ tantamount to an unsafe disjoint sum.
#
# ⟴ Look at the NEW tpc semantics.
# ⟴ “Theory {x : ℕ} extended-by {x : ℝ}” ∷ fails, is undefined; new things must have new names.
# ⟴ magmas to function-magmas via pointwise ∷ need map to do this? See Bill's papers re: IMP and instantiation.
#
# Idea: An elemented may be qualified by private and by field, why not by “parameter” as well?
...
+ [X] Switched some variationals to use symbolic names instead of strings as arguments.
      ...
      automatically “quote”ed before being passed to the underlying, 𝒱- prefixed, lisp function.
+ [X] Accounted for a clever renaming scheme: when R ↦ R′, then element names
      such as R-this automatically map to R′-this, and so are consistent
      with expected uses.
...
# + [ ] C-u C-c C-l ⇒ Generate PackageFormers but do not typecheck.
...
# Within IsSemiringWithoutAnnihilatingZero we essentially have “IsCommutativeMonoid opening (λ n → (concat "+-" n))”,
# as well as “IsMonoid opening (λ n → (concat "*-" n))”.
...
-- ∧ should allow definitions of existing names [easy, by showing-existing elements catenated in front of new extensions]
-- ∧ parse-elements seems to duplicate regexp name “_+_” as also its qualifier!
M∞  = Magma extended-by "_·_ : Set; _·_ = ⊥" ⟴ PackageFormer
M∞′ = Magma extended-by "_+_ : Set" ⟴ PackageFormer
:END:
:more_ideas:
...
[ ] Provide support for multi-line element parsing, so that 𝕀-unique looks nice!
...
[ ] Simplify 𝒱-map's “f = f” issue using the “let f = f in” trick; c.f. 𝒱-flip.

[ ] Idea: Add ;-to-lists to union that take optional pushout maps (งಠ_ಠ)ง
     ➩ Maybe union takes actual maps, while unioning takes ;-to-list so as to be consisting with renaming and opening.

[ ] In the case of “data”, ⊙, we could provide a semantics mapping.
:end:
:org-ref:
...
# ‼ User manual: Sections primed-decoration onwards need to account for equations!
...
# TODO User Manual :Incomplete: ⇨ equations!
...
<<docs('ensure)>>"
  `(let* ((ლ\(ಠ益ಠ\)ლ
           (format "700: %s\n\n\t⇨\t%s%s%s" ,message ,context
                   ...
                   (s-join "\n" (--map (format "\t⇨\t%s" (s-collapse-whitespace it))
                                       (list ,@suggestions)))))
...
{- 700
-- ➩ NEW, Support for anonymous PackageFormers.

PackageFormer the : Set where
-- 𝒱-pf decls = extended-by 'decls :adjoin-retract nil

_ = M-Set
A = Empty union (the-pf "A : Set")
-- A = the-pf "A : Set" ⟴ union (the pf "B : Set")
...
+ [ ] Crashes for some reason, why !? Somethingto do with the printing of pfs?
      ...
      This won't work, why? Something to do with printing?
       ...
       MR   = M-Set primed
       MS₃₄ = M-Set ⟴ subscripted₃

+ [ ] hom: ➩ $𝑝𝑎𝑟𝑒𝑛𝑡′ should be defined as a module opening. eek!
+ [ ] Remove *parent-context*!
  * There is an implementation key, ~*parent-context*~, which is used to
   ...
   error messages.
+ [ ] One of the allowable “types” should be any PackageFormer kind?
+ [ ] That way we have a basic form of nested modules?
+ [ ] the flattening of hierarchical structures can be formed;
+ [ ] ‼ Nesting of PackageFormers; e.g., data declarations delimited by module declarations.
...
{-
-- 𝒱-data-with-identified carrier = :kind data :level dec :alter-elements (lambda (fs) (thread-last fs (--filter (-any? (lambda (c) (s-contains? c (target (get-type it)))) carrier)) (loop for c in carrier do (--map (map-type (s-replace c $𝑛𝑎𝑚𝑒 type) it)) )))
...
-- MonoidR   =  MonoidP :kind record :waist 2 :level dec ⟴ :waist-strings ("private" "n : Set₁" "n = Set" "field")
-- MonoidD = data-with :carrier Carrier
-}

{-
𝒱-record⁷             = :kind record :waist-strings (when (package-former-elements self) '("field"))

𝒱-data-with carrier      = :kind data :level dec :alter-elements (lambda (f) (if (s-contains? carrier (target (get-type f))) (map-type (s-replace carrier $𝑛𝑎𝑚𝑒 type) f) ""))
  ...
  :alter-elements
     (λ es →
       ...
       (if discard-equations
          ...
          es)
       (--map (map-qualifier
         (λ _ → (unless (element-equations it) "field")) it) es))))
...
--
{-  lisp
(𝒱 maybemap elements = :alter-elements (lambda (fs)
...
➩ NEW variational: Extension.
  The resulting extended theory comes with a FREE view to the old, parent, theory.

➩ NEW variational: Union.
  The resulting extended theory comes with two FREE views to the old, parent, theories.

➩ NEW variational: Flipping.
  The variational supports renaming of dualised concepts.
  The resulting extended theory comes with a FREE view to the old, parent, theory.

➩ Resurrected variational: keeping.
  Essentially an opposite of extend-by.
  This lets us perform little theories after the fact; c.f. user manual part I.

➩ Fixed bug regarding barren top-level PackageFormers; e.g.,
  “PackageFormer Empty : Set₁ where” now parses correctly.

➩ Split up user manual into multiple sections.
  First section is motivational; inspired by §2 of Theory Presentation Combinators paper.
  Second section is now for more ‘advanced’ issues.

➩ A listing of variationals borne from the 5 meta-primitives is added to the user manual.

➩ Fixed bug with renaming: It was an issue with 𝒱-map.

➩ 𝒱-map now accounts for view morphisms: Items of the form “f = f” only have RHS instance altered.
  Moreover, it /optionally/ produces retract morphisms! ^_^

➩ Experimented with Emacs Lisp namespacing.
  I wish Elisp had proper modules; this issue will stall the melpa release of this system.

 ### Backend ###

 + [X] Added “λ x → ⋯” macro to reduce burden for Agda users in using Lisp.
 + [X] Typed-name string pairs have been replaced with the “element” datatype.
       Which comes with projections “element-X” and map operations “map-X”,
       as well as a number of other useful functions I needed: replace, parse, etc.
       ➩ Updated prototype to make use of this type instead of string typed-name pairs.
 + [X] Awkward “waist-strings” replaced by natural “element” datatype.
 + [X] The “:key value” syntax is no longer forced onto the user.
       ...
       ➩ The telescope style to arguments was suggested by Wolfram.
 + [X] Have begun using “declare-type” as a typing mechanism; unsure about this
       since it cannot yet handle arguments mentioning “:”, and also no convenient
       way to temporarily rid a function of a fixed type. Things to consider improving.
 + [X] Switched to /copying/ $𝒆𝑙𝑒𝑚𝑒𝑛𝑡𝑠 rather than exposing them directly, so that
 ...
 + [X] Updated nearly all of user manual to reflect the new structure of the
       prototype.
 + [X] Keeping track of a “standard library” of useful variationals to export with
       the prototype (•̀ᴗ•́)و
       ➩ They all come with useful documentation as well as some optional arguments
         for exceptional circumstances.
 + [X] New variationals:
   - [co-]decorated :: Renaming by simple provided decorations.
   - subscriptedᵢ :: Renaming by a subscript 𝒾.
 ...
 TODO:
 + [ ] Noticed ~M-Set ⟴ subscripted₃~ crashes for some reason, possibly due to a printing
       error.

 + [ ] Remove global ~*parent-context*~; it's unsued.

 + [ ] Incorporate record-with-discard-equations into user manual.
       ➩ What about all the existing variationals.
       ➩ Revise main example to include equations, as in the gpce19 paper.
         ➩ Perhaps some derived properties of M-Sets?

 + [ ] Add the necessary “quote” in the definition of 𝒱𝒸 so that arguments passed to
       other variationals need not be quoted. This is a bug that exposes how 𝓥 works!

 + [ ] Add support for \begin{lisp}   and \begin{700}
  -----------------------------------------------------------------------------------------

 [X] Formed variationals ~termtype~ and ~termtype-with-variables~ for syntactic types.
 [X] Variationals may now have optional documentation, viewable with “C-h o” ^_^
     [X] Even better, such docs are displayed as tooltips (•̀ᴗ•́)و
 [X] Elaborated code is attached to its named declaration via tooltips.
 [X] Formed variationals ~recordₑ~ and ~termtypeₑ~ that extend the non-ₑ counterparts by accounting for equations.
 [X] Paper0.agda now contains real working code for all the examples mentioned in Paper0, and then some ;-)
   - E.g., the “concatᵢ” are now generated from a *single* definition (งಠ_ಠ)ง
   - Added an appendix in response to editorial remarks.
 [X] Extended the “map” variational to accomodate PackageFormers with equations. ♥‿♥
 ...
 -----------------------------------------------------------------------------------------
   ...
   [X] Lots of cleaning at the backend.
       → *Sequential* porting currently broken.

   [X] Added two new menu items for locating variationals and PackageFormers.

   [X] Added menu item for folding away “700” and “lisp” blocks.

   [X] Begun updating Paper0

   [X] NEW, Paper0.agda for implementations of the code in Paper0.agda (•̀ᴗ•́)و
 ...
 -----------------------------------------------------------------------------------------
   ...
   [X] Updated user manual; new derived constructs include ~open, open-with~.
       These required we expose a new reference: $𝑝𝑎𝑟𝑒𝑛𝑡.
   ...
   [X] ~waist-strings~ expressions now needen't be quoted /and/ may have
       arbitrary Lisp expressions as elements (•̀ᴗ•́)و
 ...
 -----------------------------------------------------------------------------------------
   ...
   [X] Composition, ⟴, now actually composes variationals!
       Non-functional primitives are overwritten,
       alteration primitive is composed sequentially.

   [X] Introduced reference $𝑒𝑙𝑒𝑚𝑒𝑛𝑡𝑠. Used this to derive primitive ~signature~.
...
;; Stick all renames “so far” in front of each declaration.

;; (setq x a y b) ⇒ b

; (setq _xs nil)
;; (push x xs)  ⇒  updated xs

(insert (format "%s"
(funcall
(lambda (fs)
    (-as-> (-unzip (--zip-with `(,other  ,(format "let %s = %s in " (get-name it) (get-name other))) fs (--map (map-name (concat name "′") it) fs))) yup
...
(a′ : Set
 b′ : let a = a′ in Set
 c′ : let a = a′ in let b = b′ in Set)
 #+END_SRC

 TODO: Dropping after a map wont work well since the map would have created a local let; so need to remove from the lets!
 To accomodate for let-clauses we need to get the type, then drop all clauses of the form “let l = r in”
 where “l” is not in the yeses list.

 #+BEGIN_SRC emacs-lisp :var test = test :exports both :results replace :wrap "src results-agda"
;   "𝒱-primed-record = :kind record :waist-strings (\"field\")
(pf--load-variational "𝒱-primed-record = :kind 'module ⟴ :level 'inc :alter-elements (lambda (f) (map-name (concat name \"′\") f))")
(setq package-formers nil)
(load-package-former (second (get-children "PackageFormer" test)))
(load-instance-declaration "NewNamfe = M-Set primed-record ⟴ :kind 'record :waist 2 :waist-strings (\"field\")")
 #+END_SRC
...
; (load-instance-declaration "M₁  =  M-Set ⟴ primed ⟴ data :carrier \"Scalar\"" 'show-me)
(load-instance-declaration "M₂  =  M-Set ⟴ data :carrier \"Vector\" ⟴ primed" 'show-me)

(load-instance-declaration "M₁₂  =  M-Set decorated :by \"₁\" ⟴ decorated :by \"₂\"" 'show-me)
 #+END_SRC

 #+RESULTS:
 #+begin_src results-agda
{- M₁₂  =  M-Set decorated :by "₁" ⟴ decorated :by "₂" -}
PackageFormer M₁₂ : Set₁ where
   Scalar₁₂ : Set
   Vector₁₂ : Set
   _·₁₂_ : Scalar₁₂ → Vector₁₂ → Vector₁₂
   𝟙₁₂ : Scalar₁₂
 ...
 #+BEGIN_SRC emacs-lisp :var test = test :exports both :results replace :wrap "src results-agda"
(pf--load-variational "𝒱-rename expr = :alter-elements '(lambda (fs) (-as-> (-unzip (--zip-with `(,other  ,(format \"let %s = %s in \" (get-name it) (get-name other))) fs (mapcar (eval expr) fs))) yup (--zip-with (map-type (concat (s-join "" it) type) other) (-inits (cadr yup)) (car yup))))")
(setq package-formers nil)
(load-package-former (second (get-children "PackageFormer" test)))
...
(cl-defun instantiate (id)
  ...
  (letf ((self (copy-package-former (cdr (assoc (instance-declaration-package-former id) package-formers))))
        ...
        ((symbol-function '⟰)
         ...
         (lambda (c p m &optional str more) (when-let ((it (cdr (assoc (intern (format ":%s" c)) (instance-declaration-alterations id)))))
                           (unless (eval p) (error (format "700: %s\n\n\t⇨\t%s\n\t⇨\t%s ≈ %s" (eval m) (instance-declaration-docstring id) variation op)))
                           (eval `(progn ,@more))
    ...
    (unless self (error (format "700: %s\n\n\t⇨\t%s\n\t⇨\t%s ≈ %s"
                                (format "Parent “%s” not defined." (instance-declaration-package-former id))
      ...
      (⟰ 'kind '(-contains? '(record data module PackageFormer) it)
      ...
      (⟰ 'waist '(numberp it) '(format "The waist should be a number; which “%s” is not." it))

      ;; :waist-strings ≈ Extra strings to insert at the waist position.
      (⟰ 'waist-strings '(listp it) '(format "The waist-strings should be a Lisp list of strings; which “%s” is not." it))

      ;; :level ≈ Either 'inc or 'dec, for increment or decrementing the level.
      (⟰ 'level '(-contains? '(inc dec) it) '(format "The “level” must be “inc” or “dec”; which “%s” is not." it) 'string-please
  ...
  #+BEGIN_SRC emacs-lisp :var test = test :exports both :results replace :wrap "src results-agda"
;   "𝒱-primed-record = :kind record :waist-strings (\"field\")
(pf--load-variational "𝒱-primed-record = :kind record ⟴ :level nice :alter-elements (lambda (f) (map-name (concat name \"′\") f))")
(setq package-formers nil)
(load-package-former (second (get-children "PackageFormer" test)))
(let ((mid (load-instance-declaration "NewName = M-Set primed-record ⟴ :waist 2 :waist-strings (\"field\")")))
  (instantiate mid)
)
  #+END_SRC
  ...
  #+RESULTS:
  #+begin_src results-agda
{- NewName = M-Set primed-record ⟴ :waist 2 :waist-strings ("field") -}
PackageFormer NewName : Set₁ where
   Scalar  : Set
   Vector  : Set
   _·_     : Scalar → Vector → Vector
   𝟙       : Scalar
...
record Semigroup-On (Carrier : Set) : Set where
  ...
  (should (equal  (instantiate test :kind "data" :new-name "𝒮" :docstring "" :inc-level nil
...
#+END_CENTER
#+begin_quote org
This article is about implementing a prototype supporting [[https://alhassy.github.io/next-700-module-systems/][“the next 700 module systems” proposal]]
...
~open ⋯ public ⋯ renaming ⋯~ pattern which [[http://www.cse.chalmers.se/~nad/listings/lib/Algebra.Structures.html#2757][plagues]] Agda's standard library
...
the pattern of [[http://www.cse.chalmers.se/~nad/listings/lib/Algebra.Structures.html#1][defining a predicate]] ~IsX c₀ ⋯ cₙ~ on constituents ~cᵢ~
...
1. A <<<context>>> Γ is an association list, for us they consist of name-type pairs
   where later types may depend on the names of previous types:

         Γ = (x₀ : τ₀; …; xₙ : τₙ) is well-formed
     iff (x₀ : τ₀; …; xₖ₋₁ : τₖ₋₁) ⊢ τₖ Type

2. A <<<morphism>>> of contexts $σ : Γ → Δ$ is an assignment $Γ-names → Δ-terms$
   such that $Δ ⊢ σ xᵢ : σᵉ τᵢ$ where σᵉ is the homomorphic extension to types.

   For example, Γ = (A : Type; B : Type; q : A × B)
                Δ = (C : Type; p : C × C)
                σ = {A ↦ C; B → C; q ↦ p}
...
3. We have a category of contexts ℂ.

4. The subcategory of nominal assignments 𝔹 has only those morphisms whose terms
...
5. Let 𝔼 denote the category of extensions: Objects are extension morphisms
   and arrows are two 𝔹-arrows that make the resulting square commute.
...
+ ~rename-mixfix~ :: Renamaing Agda mixifix names where the rename operation
     ignores the outermost (Agda) argument position markers ‘_’.
...
#+END_SRC
#
# E.g., (pf--declare-type f₁ (:z string :w list) integer symbol string)
...
+ ~substring-delimited-here~: Given ~"⟪prefix⟫ $here ⟪suffix⟫"~
  this operation takes a string of the form  ~⋯‘prefix’⟪needle⟫‘suffix’⋯~ and yields ~needle~.
...
# (pf--substring-delimited "PackageFormer" ":") "PackageFormer 𝒫 : Set")
# (pf--substring-delimited-here "PackageFormer $here :" "PackageFormer 𝒫 : Set")
...
# E.g.,
# (rename-mixfix (lambda (it) (concat it "′")) "_⊕_")  ;; ⇒ _⊕′_
...
** “λ” for the Agda User
   :PROPERTIES:
   :CUSTOM_ID: -λ--for-the-Agda-User
   :END:

   Let's make this prototype more accessible to the Agda user
   by providing an abbreviation of Lisp functions ~(lambda (x₀ … xₙ) body)~
   into the Agda-like syntax ~(λ x₀ ⋯ xₙ → body)~.
...
The body, ~⋮~, of such a declaration consists of a number of name-type declarations
...
Tests:
#+BEGIN_SRC emacs-lisp :tangle no
;; (element-replace "R" "_≤_" (car (parse-elements '("R-reflexive : ∀ (x : U)  →  R x x" "R-reflexive = record {R = R}"))))
;;
;; (map-equations (λ eqs → (--map (replace-in-name (format "= %s.*" (regexp-quote "_^_")) "= ⊕\\1" it :is-regexp t) eqs))
;;  (element-replace "_^_" "_⊕_" (car (parse-elements '("go : That" "go = { ^ ; ^-assoc = ^-assoc}")))))

;; Tests for element-replace
;;
; Notice that the final braces are within F!
(loop for (test old new expected) in (-drop 1 (-partition 4 '("τ : Set" "τ" "ν" "ν : Set"
                                                     ...
                                                     "leftAbsorb-g1234567        : ∀ (x y : U) → x g1234567 (x + y) ≡ x"
                                                     "_g1234567_" "_↓_"
                                                     "leftAbsorb-↓ : ∀ (x y : U) → x ↓ (x + y) ≡ x"

                                                     ;; No rewriting on the RHS of record value clauses
                                                     "forget : Magma • forget = record {U = ℕ; op = _+_}"
                                                     "op" "no!"
                                                     "forget : Magma ; forget = record {U = ℕ; op = _+_}"

                                                     ;; Maintaining operands.
                                                     "forget : Magma • forget = record {U = ℕ; op = op}"
                                                     "op" "_+_"
                                                     "forget : Magma ; forget = record {U = ℕ; op = _+_}"
                                                     ...
                                                     "toX : let View X = X in View toX • toX = record {U = U;_*_ = _*_;e = e;S = S;_⟫_ = _⟫_;identity_⟫_e = identity_⟫_e}"
                                                     "e" "𝟙"
                                                     "toX : let View X = X in View toX ; toX = record {U = U;_*_ = _*_;e = 𝟙;S = S;_⟫_ = _⟫_;identity_⟫_e = identity_⟫_𝟙}"

                                                     "id : X → X • id = λ x → x"
                                                     "X" "U"
                                                     "id : U → U ; id = λ U → U" ;; .... ekk!; also arguments must be on the RHS via λ.
                                               )))
      for actual = (s-collapse-whitespace (show-element (element-replace old new (car (parse-elements (mapcar #'s-trim (s-split "•" test)))))))
      unless (equal expected actual)
...
(loop for (test . expected) in (-partition 2 '("l = record {}" "l = record {}"                    ;; zero assignments
      ...
      for actual = (element-equations (map-equations-rhs (λ x → (format " F[%s]" x)) (car (parse-elements (list "l : τ" test)))))
      unless (equal expected actual)
      do (message-box "map-equations-rhs ∷ %s ≠ %s" expected actual))
#+END_SRC
:Test:
#+BEGIN_SRC emacs-lisp :tangle no
(element-replace "e" "𝟘" (car (parse-elements '("go : These" "go = {e = e ; e2 = e2}"))))

(equal "*-comm" (replace-in-name "_+_"  "_*_" "+-comm"))
(replace-in-name "e"  "𝟘" "e2")

;; Identity -- OH NO! “+” is also considered a qualifier! This is due to parse elements!
(element-replace "_+_" "_+_" (car (parse-elements '("_+_ : U → U → U" "_+_ = let _^_ = ^-comm in ⋯ + ⋯ ^ ⋯"))))
...
;; Single letter
(element-replace "R" "_≤_" (car (parse-elements '("r-R-reflexive : ∀ (x : U)  →  R x x" "r-R-reflexive = record{l = r}"))))

;; In equations?
(element-replace "_^_" "_⊕_" (car (parse-elements '("_+_ : U → U → U" "_+_ = let _^_ = ^-comm in ⋯ + ⋯ * ⋯"))))

;; In a type?
(element-replace "_*_" "_+_" (car (parse-elements '("*-comm : ∀ x y → x * y ≡ y * x"))))

;; In a type? Not altering part of names.
(element-replace "inv" "neg" (car (parse-elements '("inv : Id Involutionary"))))


(replace-regexp-in-string (format "%s\\([^[:alnum:]]\\)" "R") (format  "%s\\1" "≤") "R-reflexive : ∀ (x : U)  →  R x x")

(element-replace "_·_" "_+_" (car (parse-elements '("toMonoidR       : let View X = X in View MonoidR" "toMonoidR = record {Carrier = Carrier;_·_ = _·_;assoc = assoc;𝕀 = 𝕀;leftId = leftId;rightId = rightId}"))))
...
(-let [e (car (parse-elements '("toMonoidR : let View X = X in View MonoidR"
                                "toMonoidR = record {Carrier = Carrier;_+_ = _+_;assoc = assoc;𝕀 = 𝕀;leftId = leftId;rightId = rightId}")))]
...
(mapcar #'parse-name '("A : Set" "private B : Set" "C : ℕ → Set" "B = A" "C zero = A" "C (suc n) = A" "D : Set" "E : {a b : Scalar} {𝓋 : Vector} → (a × b) · 𝓋  ≡  a · (b · 𝓋)))" "𝒮ℯ𝓉 = Set"))
#+END_SRC
...
'("A : Set" "private B : Set" "C : ℕ → Set" "B = A" "C zero = A" "C (suc n) = A" "D : Set" "E : {a b : Scalar} {𝓋 : Vector} → (a × b) · 𝓋  ≡  a · (b · 𝓋)))")))
#+END_SRC

#+RESULTS:
| #s(element nil A Set nil)                                                         |
| #s(element private  B Set (B = A))                                                |
| #s(element nil C ℕ → Set (C zero = A C (suc n) = A))                              |
| #s(element nil D Set nil)                                                         |
| #s(element nil E {a b : Scalar} {𝓋 : Vector} → (a × b) · 𝓋  ≡  a · (b · 𝓋))) nil) |
:End:
...
(("Hom²" . #s(package-former "Hom² = M-Set-R hom ⟴ renaming :by \"map₁ to scalar; pres-𝟙 to unity\"" "record" "Hom²" "₁" 2
                             ("open M-Set-R  Src" "open M-Set-R′ Tgt" "field")
                             3
                             ("Src : M-Set-R" "Tgt : M-Set-R" "scalar : Scalar → Scalar′" "map₂ : Vector → Vector′" "pres-· : {x₁ : Scalar} → {x₂ : Vector} →   map₂ (_·_ x₁ x₂)   ≡   _·′_ (scalar x₁) (map₂ x₂)" "unity : scalar (𝟙 )   ≡   𝟙′" "pres-× : {x₁ : Scalar} → {x₁ : Scalar} →   scalar (_×_ x₁ x₁)   ≡   _×′_ (scalar x₁) (scalar x₁)")))
 ("Hom" . #s(package-former "Hom  = M-Set-R hom" "record" "Hom" "₁" 2
                            ("open M-Set-R  Src" "open M-Set-R′ Tgt" "field")
                            3
                            ("Src : M-Set-R" "Tgt : M-Set-R" "map₁ : Scalar → Scalar′" "map₂ : Vector → Vector′" "pres-· : {x₁ : Scalar} → {x₂ : Vector} →   map₂ (_·_ x₁ x₂)   ≡   _·′_ (map₁ x₁) (map₂ x₂)" "pres-𝟙 : map₁ (𝟙 )   ≡   𝟙′" "pres-× : {x₁ : Scalar} → {x₁ : Scalar} →   map₁ (_×_ x₁ x₁)   ≡   _×′_ (map₁ x₁) (map₁ x₁)")))
 ("MonSig" . #s(package-former "MonSig = M-Set record ⟴ signature" "record" "MonSig" "₁" 0
                               ("field")
                               3
                               ("Scalar  : Set" "Vector  : Set" "_·_     : Scalar → Vector → Vector" "𝟙       : Scalar" "_×_     : Scalar → Scalar → Scalar")))
 ("MonoidSignature" . #s(package-former "MonoidSignature = M-Set record ⟴ generated :by (lambda (f) (and (s-contains? \"Scalar\" f) (not (s-contains? \"Vector\" f))))" "record" "MonoidSignature" "₁" 0
                                        ("field")
                                        3
                                        ("Scalar  : Set" "𝟙       : Scalar" "_×_     : Scalar → Scalar → Scalar")))
 ("M-Set-Sorts" . #s(package-former "M-Set-Sorts = M-Set record ⟴ sorts" "record" "M-Set-Sorts" "₁" 0
                                    ("field")
                                    3
                                    ("Scalar  : Set" "Vector  : Set")))
 ("M-Set-R′" . #s(package-former "M-Set-R′ = M-Set-R open-with :decoration \"′\"" "module" "M-Set-R′" none 1
                                 ("")
                                 3
                                 ("ℛ : M-Set-R" "open M-Set-R ℛ public" "  renaming" "    ( Scalar to Scalar′\n       ; Vector to Vector′\n       ; _·_ to _·′_\n       ; 𝟙 to 𝟙′\n       ; _×_ to _×′_\n       ; leftId to leftId′\n       ; assoc to assoc′\n       )")))
 ("M-Set-R₁" . #s(package-former "M-Set-R₁ = M-Set-R open :with (lambda (x) (concat x \"₁\"))" "module" "M-Set-R₁" none 1
                                 ("")
                                 3
                                 ("ℛ : M-Set-R" "open M-Set-R ℛ public" "  renaming" "    ( Scalar to Scalar₁\n       ; Vector to Vector₁\n       ; _·_ to _·₁_\n       ; 𝟙 to 𝟙₁\n       ; _×_ to _×₁_\n       ; leftId to leftId₁\n       ; assoc to assoc₁\n       )")))
 ("M-Set-R" . #s(package-former "M-Set-R = M-Set record" "record" "M-Set-R" "₁" 0
                                ("field")
                                3
                                ("Scalar  : Set" "Vector  : Set" "_·_     : Scalar → Vector → Vector" "𝟙       : Scalar" "_×_     : Scalar → Scalar → Scalar" "leftId  : {𝓋 : Vector}  →  𝟙 · 𝓋  ≡  𝓋" "assoc   : {a b : Scalar} {𝓋 : Vector} → (a × b) · 𝓋  ≡  a · (b · 𝓋)")))
 ("Neato" . #s(package-former "Neato = M-Set empty-module" "module" "Neato" none 999 nil 3
                              ("Scalar  : Set" "Vector  : Set" "_·_     : Scalar → Vector → Vector" "𝟙       : Scalar" "_×_     : Scalar → Scalar → Scalar" "leftId  : {𝓋 : Vector}  →  𝟙 · 𝓋  ≡  𝓋" "assoc   : {a b : Scalar} {𝓋 : Vector} → (a × b) · 𝓋  ≡  a · (b · 𝓋)")))
 ("NearMonoid¹" . #s(package-former "NearMonoid¹ = M-Set record ⟴ single-sorted :with-sort \"Carrier\"" "record" "NearMonoid¹" "₁" 0
                                    ("field")
                                    3
                                    ("Carrier : Set" "_·_ : Carrier → Carrier → Carrier" "𝟙 : Carrier" "_×_ : Carrier → Carrier → Carrier" "leftId : {𝓋 : Carrier}  →  𝟙 · 𝓋  ≡  𝓋" "assoc : {a b : Carrier} {𝓋 : Carrier} → (a × b) · 𝓋  ≡  a · (b · 𝓋)")))
 ("NearMonoid" . #s(package-former "NearMonoid = M-Set record ⟴ renaming :by \"Scalar to Carrier; Vector to Carrier; · to ×\"" "record" "NearMonoid" "₁" 0
                                   ("field")
                                   3
                                   ("Carrier : Set" "_×_ : Carrier → Carrier → Carrier" "𝟙 : Carrier" "leftId : {𝓋 : Carrier}  →  𝟙 × 𝓋  ≡  𝓋" "assoc : {a b : Carrier} {𝓋 : Carrier} → (a × b) × 𝓋  ≡  a × (b × 𝓋)")))
 ("MRₜₒ_" . #s(package-former "MRₜₒ_ = M-Set record ⟴ renaming_ :by \"Scalar to S; Vector to V; _·_ to _nice_\"" "record" "MRₜₒ_" "₁" 0
                              ("field")
                              3
                              ("S : Set" "V : Set" "_nice_ : S → V → V" "𝟙 : S" "_×_ : S → S → S" "leftId : {𝓋 : V}  →  𝟙 nice 𝓋  ≡  𝓋" "assoc : {a b : S} {𝓋 : V} → (a × b) nice 𝓋  ≡  a nice (b nice 𝓋)")))
 ("MRₜₒ" . #s(package-former "MRₜₒ = M-Set record ⟴ renaming :by \"Scalar to S; Vector to V; · to nice\"" "record" "MRₜₒ" "₁" 0
                             ("field")
                             3
                             ("S : Set" "V : Set" "_nice_ : S → V → V" "𝟙 : S" "_×_ : S → S → S" "leftId : {𝓋 : V}  →  𝟙 nice 𝓋  ≡  𝓋" "assoc : {a b : S} {𝓋 : V} → (a × b) nice 𝓋  ≡  a nice (b nice 𝓋)")))
 ("MR-oh" . #s(package-former "MR-oh  = M-Set record ⟴ rename :elements (lambda (name) (pcase name (\"Scalar\" \"S\") (x x)))" "record" "MR-oh" "₁" 0
                              ("field")
                              3
                              ("S : Set" "Vector : Set" "_·_ : S → Vector → Vector" "𝟙 : S" "_×_ : S → S → S" "leftId : {𝓋 : Vector}  →  𝟙 · 𝓋  ≡  𝓋" "assoc : {a b : S} {𝓋 : Vector} → (a × b) · 𝓋  ≡  a · (b · 𝓋)")))
 ("the-MR" . #s(package-former "the-MR   = M-Set record ⟴ co-decorated :by \"the-\"" "record" "the-MR" "₁" 0
                               ("field")
                               3
                               ("the-Scalar : Set" "the-Vector : Set" "_the-·_ : the-Scalar → the-Vector → the-Vector" "the-𝟙 : the-Scalar" "_the-×_ : the-Scalar → the-Scalar → the-Scalar" "the-leftId : {𝓋 : the-Vector}  →  the-𝟙 the-· 𝓋  ≡  𝓋" "the-assoc : {a b : the-Scalar} {𝓋 : the-Vector} → (a the-× b) the-· 𝓋  ≡  a the-· (b the-· 𝓋)")))
 ("MR₁₋₂" . #s(package-former "MR₁₋₂    = M-Set record ⟴ decorated :by \"₁\" ⟴ decorated :by \"₂\"" "record" "MR₁₋₂" "₁" 0
                              ("field")
                              3
                              ("Scalar₁₂ : Set" "Vector₁₂ : Set" "_·₁₂_ : Scalar₁₂ → Vector₁₂ → Vector₁₂" "𝟙₁₂ : Scalar₁₂" "_×₁₂_ : Scalar₁₂ → Scalar₁₂ → Scalar₁₂" "leftId₁₂ : {𝓋 : Vector₁₂}  →  𝟙₁₂ ·₁₂ 𝓋  ≡  𝓋" "assoc₁₂ : {a b : Scalar₁₂} {𝓋 : Vector₁₂} → (a ×₁₂ b) ·₁₂ 𝓋  ≡  a ·₁₂ (b ·₁₂ 𝓋)")))
 ("MR′" . #s(package-former "MR′ = M-Set record ⟴ primer" "record" "MR′" "₁" 0
                            ("field")
                            3
                            ("Scalar′  : Set" "Vector′  : Set" "_·′_     : Scalar′ → Vector′ → Vector′" "𝟙′       : Scalar′" "_×′_     : Scalar′ → Scalar′ → Scalar′" "leftId′  : {𝓋 : Vector′}  →  𝟙′ ·′ 𝓋  ≡  𝓋" "assoc′   : {a b : Scalar′} {𝓋 : Vector′} → (a ×′ b) ·′ 𝓋  ≡  a ·′ (b ·′ 𝓋)")))
 ("Monoidₙ" . #s(package-former "Monoidₙ = MonoidR rename₁ :elements (lambda (name) (concat name \"ₙ\"))" "record" "Monoidₙ" "₁" 0
                                ("field")
                                4
                                ("Carrierₙ : Set" "_⨾ₙ_ : let Carrier = Carrierₙ in Carrier → Carrier → Carrier" "Idₙ : let Carrier = Carrierₙ in let _⨾_ = _⨾ₙ_ in Carrier" "assocₙ : let Carrier = Carrierₙ in let _⨾_ = _⨾ₙ_ in let Id = Idₙ in ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftIdₙ : let Carrier = Carrierₙ in let _⨾_ = _⨾ₙ_ in let Id = Idₙ in let assoc = assocₙ in ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightIdₙ : let Carrier = Carrierₙ in let _⨾_ = _⨾ₙ_ in let Id = Idₙ in let assoc = assocₙ in let leftId = leftIdₙ in ∀ {x : Carrier} → x ⨾ Id ≡ x")))
 ("Monoidₘ" . #s(package-former "Monoidₘ = MonoidR map₀ :elements (lambda (f) (make-tn (concat (get-name f) \"ₘ\") (get-type f)))" "record" "Monoidₘ" "₁" 0
                                ("field")
                                4
                                ("Carrierₘ : Set" "_⨾_ₘ : let Carrier = Carrierₘ in Carrier → Carrier → Carrier" "Idₘ : let Carrier = Carrierₘ in let _⨾_ = _⨾_ₘ in Carrier" "assocₘ : let Carrier = Carrierₘ in let _⨾_ = _⨾_ₘ in let Id = Idₘ in ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftIdₘ : let Carrier = Carrierₘ in let _⨾_ = _⨾_ₘ in let Id = Idₘ in let assoc = assocₘ in ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightIdₘ : let Carrier = Carrierₘ in let _⨾_ = _⨾_ₘ in let Id = Idₘ in let assoc = assocₘ in let leftId = leftIdₘ in ∀ {x : Carrier} → x ⨾ Id ≡ x")))
 ("MonoidR″" . #s(package-former "MonoidR″   =  MonoidR primedₗₑₜ" "record" "MonoidR″" "₁" 0
                                 ("field")
                                 4
                                 ("Carrier′ : Set" "_⨾′_ : let Carrier = Carrier′ in Carrier → Carrier → Carrier" "Id′ : let Carrier = Carrier′ in let _⨾_ = _⨾′_ in Carrier" "assoc′ : let Carrier = Carrier′ in let _⨾_ = _⨾′_ in let Id = Id′ in ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftId′ : let Carrier = Carrier′ in let _⨾_ = _⨾′_ in let Id = Id′ in let assoc = assoc′ in ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightId′ : let Carrier = Carrier′ in let _⨾_ = _⨾′_ in let Id = Id′ in let assoc = assoc′ in let leftId = leftId′ in ∀ {x : Carrier} → x ⨾ Id ≡ x")))
 ("MonoidR′" . #s(package-former "MonoidR′   =  MonoidP record ⟴ primedₗₑₜ" "record" "MonoidR′" "₁" 0
                                 ("field")
                                 4
                                 ("Carrier′ : Set" "_⨾′_ : let Carrier = Carrier′ in Carrier → Carrier → Carrier" "Id′ : let Carrier = Carrier′ in let _⨾_ = _⨾′_ in Carrier" "assoc′ : let Carrier = Carrier′ in let _⨾_ = _⨾′_ in let Id = Id′ in ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftId′ : let Carrier = Carrier′ in let _⨾_ = _⨾′_ in let Id = Id′ in let assoc = assoc′ in ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightId′ : let Carrier = Carrier′ in let _⨾_ = _⨾′_ in let Id = Id′ in let assoc = assoc′ in let leftId = leftId′ in ∀ {x : Carrier} → x ⨾ Id ≡ x")))
 ...
 ("M-Set-Typeclass₃" . #s(package-former "M-Set-Typeclass₃ = M-Set-Record typeclass :height 3 :level 'dec" "record" "M-Set-Typeclass₃" "" 3
                                         ("field")
                                         3
                                         ("Scalar  : Set" "Vector  : Set" "_·_     : Scalar → Vector → Vector" "𝟙       : Scalar" "_×_     : Scalar → Scalar → Scalar" "leftId  : {𝓋 : Vector}  →  𝟙 · 𝓋  ≡  𝓋" "assoc   : {a b : Scalar} {𝓋 : Vector} → (a × b) · 𝓋  ≡  a · (b · 𝓋)")))
 ("M-Set-Record" . #s(package-former "M-Set-Record = M-Set record" "record" "M-Set-Record" "₁" 0
                                     ("field")
                                     3
                                     ("Scalar  : Set" "Vector  : Set" "_·_     : Scalar → Vector → Vector" "𝟙       : Scalar" "_×_     : Scalar → Scalar → Scalar" "leftId  : {𝓋 : Vector}  →  𝟙 · 𝓋  ≡  𝓋" "assoc   : {a b : Scalar} {𝓋 : Vector} → (a × b) · 𝓋  ≡  a · (b · 𝓋)")))
 ("MonoidT₄" . #s(package-former "MonoidT₄   =  MonoidP typeclass :height 4 :level 'dec" "record" "MonoidT₄" "" 4
                                 ...
                                 4
                                 ("Carrier : Set" "_⨾_     : Carrier → Carrier → Carrier" "Id      : Carrier" "assoc   : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftId  : ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightId : ∀ {x : Carrier} → x ⨾ Id ≡ x")))
 ("MonoidT₂" . #s(package-former "MonoidT₂   =  MonoidP typeclass₂ ⟴ :waist-strings (\"private\" \"extra : Set₁\" \"extra = Set\" \"field\")" "record" "MonoidT₂" "" 2
                                 ("private" "extra : Set₁" "extra = Set" "field")
                                 4
                                 ("Carrier : Set" "_⨾_     : Carrier → Carrier → Carrier" "Id      : Carrier" "assoc   : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftId  : ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightId : ∀ {x : Carrier} → x ⨾ Id ≡ x")))
 ("MonoidT₃" . #s(package-former "MonoidT₃   =  MonoidP record ⟴ :waist 3 :level dec" "record" "MonoidT₃" "" 3
                                 ...
                                 4
                                 ("Carrier : Set" "_⨾_     : Carrier → Carrier → Carrier" "Id      : Carrier" "assoc   : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftId  : ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightId : ∀ {x : Carrier} → x ⨾ Id ≡ x")))
 ("MonoidPᶜ" . #s(package-former "MonoidPᶜ = MonoidP ⟴" "PackageFormer" "MonoidPᶜ" "₁" 0 nil 4
                                 ("Carrier : Set" "_⨾_     : Carrier → Carrier → Carrier" "Id      : Carrier" "assoc   : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftId  : ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightId : ∀ {x : Carrier} → x ⨾ Id ≡ x")))
 ...
 ("MonoidPⁱᵈ" . #s(package-former "MonoidPⁱᵈ = MonoidP identity" "PackageFormer" "MonoidPⁱᵈ" "₁" 0 nil 4
                                  ("Carrier : Set" "_⨾_     : Carrier → Carrier → Carrier" "Id      : Carrier" "assoc   : ∀ {x y z} → (x ⨾ y) ⨾ z ≡ x ⨾ (y ⨾ z)" "leftId  : ∀ {x : Carrier} → Id ⨾ x ≡ x" "rightId : ∀ {x : Carrier} → x ⨾ Id ≡ x")))
 ("M-Set" . #s(package-former nil "PackageFormer" "M-Set" "₁" 0 nil 3
                              ("Scalar  : Set" "Vector  : Set" "_·_     : Scalar → Vector → Vector" "𝟙       : Scalar" "_×_     : Scalar → Scalar → Scalar" "leftId  : {𝓋 : Vector}  →  𝟙 · 𝓋  ≡  𝓋" "assoc   : {a b : Scalar} {𝓋 : Vector} → (a × b) · 𝓋  ≡  a · (b · 𝓋)")))
 (porting . "")
...
➩ NEW, Pushout construction!

  𝒱-union forms pushouts in the category of signatures ^_^
...
➩ NEW, Pullback construction ^_^

  This is much harder to do than pushout; but shares the same ideas for reducing the number of input arguments.

➩ “to”-lists can now be interpreted bijectively!

   𝒱-renaming now has an optional parameter “:adjoin-coretract” to form
   the other half of the bijection. Unlike “:adjoin-retract”, it's off by default.

➩ Add remarks on commutativity of combinators in user manual section on renaming.

➩ NEW variational, “postulating”.
...
➩ Redirected extraction of 𝒱ariationals defined in the first part of the user manual
  via reference 𝒱-user-man-pt-1.

  Since they relied on the variationals defined in part 2, they caused an error during compilation.

➩ Fixed indentation issue; using at least 4 spaces in elaborated code.

➩ Support crazy names, like [],+,\,^,*,?

➩ Experimented with having anonymous PackageFormers.

➩ Made support-for-mixfix names off by default.

➩ Added adjoin-retract option to 𝒱-extended-by.

➩ Fixed tooltip calculation to account for regular expression operators, such as [],+,\,^,*,?.
...
+ [X] PackageFormer.ext ↦ package-former.ext

    ⇨ kebab-case is conventional of Lisp
+ [X] Arguments, in tooltip renditions, are in reverse order; c.f. 𝒱-open.
...
** Equality: From ≡ to ≈ to ≈′
   ...
   :END:
   One may start from a basic ≡-equality, then *mechanically* move to a setoid one
...
** COMMENT DONE Extensions
   ...
   *Theory graphs:*
   ...
   between structures. Such “mathematical knowledge management” is underway with the MMT setup.
...
** TODO Works to look into
 ...
 + [ ]  J. Carette and W. M. Farmer. High-level theories. In A. Autexier et al., editor,
        ...
        Science, pages 232–245. Springer-Verlag, 2008.
...
:Hide:
   ...
   This must come at the end since it uses things like 𝒱 which need to be defined first.

   #  :var it = (s-count-matches-all "(𝒱" (file-as-string "variationals.tmp"))
# (setq ♯standard-variationals 10)
...
{-700
PackageFormer this : Set₁ where
  A : Set
  a : A

𝒱-mid = "neato"

thisR = this record ⟴ mid
-}
...
(cl-defun parse-labelled-to-list (label the-list &key (no-to nil))
     ...
     (setq otherwise (thread-last (car (-take-last 1 result))
           (s-replace "++" " ")
           (pf--substring-delimited-here "λ $here")
           (s-split " → ")
...
** Namespacing                                        :experimenting:
 ...
 # Warning: Many instances instance of “defun” are actually “cl-defun” in disguise!
 # This is because the name-space hack currently used is wrapped in an “eval”,
 # which consequently requires _all_ non-defun declarations to be quoted.

 #+BEGIN_SRC emacs-lisp :tangle agda-next-700-module-systems.el
;; The “eval” requires all non-defun declarations to be quoted;
