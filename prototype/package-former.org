...
#+END_SRC
   ...
   Unlike ~𝒱-identity~ from the previous section, we have decided to split this definition into multiple
...
but is in-fact a homomorphism: ~exposing n ⟴ exposing m ≈ exposing (n + m)~.
#+BEGIN_SRC lisp :tangle "variationals.tmp"
(𝒱 exposing n
