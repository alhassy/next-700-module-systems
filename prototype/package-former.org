...
# ⟴ magmas to function-magmas via pointwise ∷ need map to do this? See Bill's papers re: IMP and instantation.
#
...
#+END_SRC
   ...
   ~f : List Elements → List Elements~ and ~Γ,x₀:τ₀,…,xₖ₋₁:τₖ₋₁ ⊢ τₖ Type~ for ~k : 0..n~.
...
(element-replace "R" "_≤_" (car (parse-elements '("R-reflexive : ∀ (x : U)  →  R x x"))))
#+END_SRC
#+BEGIN_SRC emacs-lisp
