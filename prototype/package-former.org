...
open import package-former_Generated
open import Level
...
#+begin_src emacs-lisp :tangle yes :results replace
(cl-defun pf--parse-variational-clause (line)
...
;; TODO: Might as well throw away pf--load-instance-declaration now.
;
(pf--parse-variational-clause ":waist 2 ⟴ :kind record ⟴ more :yarg x :barg y")
#+end_src
...
(cl-defun instantiate (decls &key
  ...
  "Given a PackageFormer declaration, instantiate it into a concrete Agda type.
   ...
   - ‘decls’ is immediately provided to ‘pf--get-children’, so it may be a string
      or a list.
  ...
  (let* ((pf (cadr (pf--get-children "PackageFormer" decls)))
     (header     (car pf))
     ...
     (indentation (s-repeat (if fields (pf--get-indentation (car fields)) 0) " "))
...
(let (package-formers instantiations-remaining pf--annotations porting-list variationals)
