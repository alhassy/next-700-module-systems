...
#    This is becuase 𝒱 lists them in the wrong order!
...
(load-variational "𝒱-primed-record = :kind 'module ⟴ :level 'inc :alter-elements (lambda (f) (map-name (concat name \"′\") f))")
...
PackageFormer M₁₂ : Set₁ where
 ...
 #+BEGIN_SRC emacs-lisp :var test = test :exports both :results replace :wrap "src results-agda"
(load-variational "𝒱-rename expr = :alter-elements '(lambda (fs) (-as-> (-unzip (--zip-with `(,other  ,(format \"let %s = %s in \" (get-name it) (get-name other))) fs (mapcar (eval expr) fs))) yup (--zip-with (map-type (concat (s-join "" it) type) other) (-inits (cadr yup)) (car yup))))")
(setq package-formers nil)
(load-package-former (second (get-children "PackageFormer" test)))
...
(cl-defun instantiate (id)
  ...
  #+BEGIN_SRC emacs-lisp :var test = test :exports both :results replace :wrap "src results-agda"
;   "𝒱-primed-record = :kind record :waist-strings (\"field\")
(load-variational "𝒱-primed-record = :kind record ⟴ :level nice :alter-elements (lambda (f) (map-name (concat name \"′\") f))")
(setq package-formers nil)
(load-package-former (second (get-children "PackageFormer" test)))
...
** First-class PackageFormers
   :PROPERTIES:
   :CUSTOM_ID: Composition-~⟴~-and-First-class-PackageFormers
   :END:
...
𝒱-generated by = :alter-elements (lambda (fs) (-filter by fs))
#+END_SRC
...
** Automatically deriving homomorphism definitions ♥‿♥          :out_of_date:
   :PROPERTIES:
   :CUSTOM_ID: Automatically-deriving-homomorphism-definitions-♥‿♥
   :END:
...
(𝒱 hom
  = record ⟴
    :waist 2
    :waist-strings '("open M-Set-R  Src" "open M-Set-R′ Tgt" "field")
    :alter-elements (lambda (es)
...
{-700
Hom  = M-Set hom
Hom² = M-Set hom ⟴ renaming :by "map₁ to scalar; pres-𝟙 to unity"
-}
...
#+END_SRC
#
# E.g., (declare-type f₁ (:z string :w list) integer symbol string)
...
# (substring-delimited "PackageFormer" ":") "PackageFormer 𝒫 : Set")
# (substring-delimited-here "PackageFormer $here :" "PackageFormer 𝒫 : Set")
...
** ~Itify~: A macro that makes macros ♥‿♥
   :PROPERTIES:
   :CUSTOM_ID: ~Itify~:-A-macro-that-makes-macros-♥‿♥
   :END:
...
(cl-defun parse-labelled-to-list (label the-list &key (no-to nil))
     ...
     (setq otherwise (thread-last (car (-take-last 1 result))
           (s-replace "++" " ")
           (substring-delimited-here "λ $here")
           (s-split " → ")
