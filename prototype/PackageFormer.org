...
# [‼] Nesting of PackageFormers; e.g., data declarations delimited by module declara tions.
#
...
#+BEGIN_src results-agda
{- 𝒱-primed-attempt  = :kind record :waist-strings ("field") :alter-elements (lambda(f) (map-name (concat name \"′\") f))
...
#+END_src
   ...
   1. The system comes with a Lisp methods ~map-name~ and ~map-type~ that yield the name and type part,
      ...
      Upon invocation, their second argument may mention /symbols/ ~name~, or ~type~ respectively, to refer to
      that part of their third argument ---the typed-name ~f~ above in the definition of ~𝒱-primed-attempt~.
   ...
   3. One would expect catenating a prime to the mixfix name ~_×_~ would yield ~_×_′~ but above
      ...
      ~map-name~ performs a rewrite to a name by jumping over the Agda mixfix marker, ~_~,
...
using the special identifier ~$𝑛𝑎𝑚𝑒~. In a ~lisp~ comment, we formalise this algorithm as follows.
#+BEGIN_src emacs-lisp
(𝒱 data carrier
  = :kind data
    :level dec
...
#+END_src
   ...
   preprended with ~let x = x′ in~ to propagate this change.
...
(let* ((raw (cadr (get-children "PackageFormer" test)))
  ...
  (setf (package-former-waist pf) 2)
   (show-package-former pf
    :omit-car-element t
    :extra-waist-strings (list "." "private" "n : ℕ" "n = 3" "top-level 𝒮ℯ𝓉 = Set" "field {- elements -} "))
   ))
#+END_SRC
