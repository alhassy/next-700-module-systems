...
| Status | Instantiation for latest notion of =Context=-s not working |
↪ See the very bottom of this article.

# (shell-command "ln -s theory.org readme.md")
...
** TODO Modus Ponens … ⁉                                               :Help:
Unfortunately, since ~B, C, D~ are all of type ~Set₁~ and so are not functions,
there is no trivial way to “instantiate” any of ~B, C, D~ to obtain versions of ~A~
wherein certain fields are already “fixed”.

I'd like to write =B on ℕ=, for example. However, the definition of ~_on_~ seems to
want me to pattern match on sorts.
  #+begin_src agda2 :tangle semantics-with-waist.agda
B-on-ℕ : Set
