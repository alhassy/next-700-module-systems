...
** Document class options                                            :ignore:
  ...
  #+latex_class_options: [review,anonymous]
...
* Introduction
  ...
  if we know that ~ceq : M₁ ≡ M₂~ then it is “obvious” that
  ~Id₂ ⨾₂ (x ⨾₁ Id₁) ≡ x~ for all
  ...
  The resulting formula then typechecks, but is hideous.
  ``subst hell'' only gets worse.  Below, we use pointed magmas for brevity,
...
module Akward-Formulation (A B : Magma₀)
...
module Nicer
  ...
  concept, such as homomorphism, which are awkward in this way, and cleaner
...
So not only are there no general rules for when the bundle or not, it is
