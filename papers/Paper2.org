...
↝{- Termtypes are fixpoints of type constructors. -}
 ...
 Since we may view an
  ...
  For example, considered as a unary constructor ~op : A → B~
  targets the type termtype ~B~ and so its source is ~A~.
...
*** Decreasing Debrujin Indices
Given a quantification ~(⊕ x ∶ τ • fx)~, its body ~fx~ may refer to a free variable
~x~.  If we decrement all Debrujin indices ~fx~ contains, then there would be no
reference to ~x~.
