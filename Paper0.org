...
# JC: Using a hard-coded path for resources like this is EVIL. Please fix.
...
* Introduction --- Selecting the ‘right’ perspective
  ...
  in Haskell, is only allowed one instance per datatype. The Booleans,
  however, support multiple monoid instances such as sequential and parallel monoids
  ---the former being conjunction with
  identity /true/ and the latter being disjunction with identity /false/.

  The solution, in Haskell, is to have two isomorphic copies ~All~ 
  (using conjunction) and ~Any~ (using disjunction).
  An alternative solution would be to parameterise the monoid interface by
  its underlying compositionality scheme as well as its carrier type.
  ...
  we will illustrate our ideas in Agda~\cite{}. For the example at hand,
