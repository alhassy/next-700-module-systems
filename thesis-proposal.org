...
#+LATEX_HEADER: \newunicodechar{′}{'}
#+LATEX_HEADER: \newunicodechar{⦃}{ \{\!\mid }}  % this is not correct 
#+LATEX_HEADER: \newunicodechar{⦄}{ \mid\!\} }   % this is not correct 
...
Alternatively, in a DTL we may encode the monoidal interface using dependent products directly rather than use the syntactic sugar
of records. The notation ~Σ x ∶ A • B x~ denotes the type of pairs ~(x , pf)~ where ~x : A~ and ~pf : B x~ ---i.e., a record consisting of two fields.
...
and so when it is used in ~pop-Id-Σₜ~ we project to the desired components.
...
Since these are not top level items by themselves, we position them in a ~module~
